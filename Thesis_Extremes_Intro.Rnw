...
However, the annual heavy precipitation events have increased globally in a disproportional way compared to mean changes between 1951 and 2003 over various mid-latitude areas, even in regions where a reduction was found in the mean \citep{IPCC2013}. This difference in changes between mean and extreme precipitation can be explained from the fact that the global-mean precipitation is constrained by the energy budget of the atmosphere, while the extreme events depend in a non-linear way on the moisture availibility \citep{Pall2007,IPCC2013}. %Whereas global-mean precipitation is mainly determined by the global energy budget, extreme events are constrainted in intensity by moisture availability \citep{Pall2007}.
...
Next to temperature, dewpoint temperature and vertical instability, the degree of wind shear can be a factor of importance for trends in extreme precipitation \citep{Brauer2007,Trenberth2011}. Wind shear can namely influence the intensity of precipitation via the organisation of precipitation sytems \citep{Browning1964,Sumner1988,Weisman1982,Wingo2010}.
...
This thesis aims at understanding how extreme hourly precipitation in the Netherlands change in time and what the mechanism is behind these changes. In a threefold way we investigated this by: (i) analyzing seasonal and spatial characterstics of hourly precipitation, (ii) applying trend analysis on 2-day maxima in hourly precipitation intensity, 2-day frequencies of wet hours and 2-day precipitation sums for 5 stations, and (iii) deriving key variables for changes in extreme precipitation from theory and strength of correlation and thorougly investigating time related changes for the maxima of the key variables themselves and for their relationships with precipitation maxima. The present work concentrates on 5 Dutch stations; De Kooy, De Bilt, Eelde, Vlissingen and Maastricht.
...
the intensity and frequency of wet hours and the intensity of extremes. Consequently we decide that it is relevant to study the trends of hourly precipitation extremes for the 5 stations in the Netherlands seperately instead of bundling the station data to enhance the chance on trend detection. %in order to receive higher sample size .
We also defined two key seasons based on the seasonal dependance of the precipitation characterstics. This spatial analysis combined with differentiating on season, provides more detailled and robust knowledge about changes in hourly precipitation. In Chapter 5 we present the results of trend analysis in 2-day intensity, frequency and precipitation sum of extremely wet hours between 1958-2016 for the 5 stations. Both linear regression, and quantile regression is applied. Based on how the data is distributed these regression techniques are used solely or in combination. The significance of all regression fits is investigated in a 9999 Monte Carlo Permutation test. The fifth part (Chapter 6) tends to identify and attribute variables responsible for the detected trends in Chapter 5. % we zoomed in on the mechanism behind the observed trends in hourly precipitation.
We defined key variables based on theory and their strength of correlation with precipitation maxima. Furthermore, time regression is applied on temperature, dewpoint temperature, Convective Available Potential Energy (CAPE) and wind speed to observe if their trends match the trends found for precipitation intensity. Besides, we verify whether the super CC-scaling found by \citet{Lenderink2008,Lenderink2009,Lenderink2011} is present in our hourly data and if it may be assigned to mixing of winter-and-summer-dominated processes only or to timewise changes of the climate state. Additionally we investigate whether changes in precipitation intensity maxima are the result of changes in the distribution(s) of the key variable(s) and/or whether these are the result of changes in the relationship of precipitation intensity with the key variable(s). In Chapter 7 the present work is extended with a case study into 2 observational records from Norway and Spain, in order to find out if we detect similar trends in precipitation extremes and find the same relationship with temperature. Lastly, we end with a discussion of the method and results and present the main conclusions.
