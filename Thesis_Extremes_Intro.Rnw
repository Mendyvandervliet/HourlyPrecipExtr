...
the intensity and frequency of wet hours and the intensity of extremes. Consequently we decide that it is relevant to study the trends of hourly precipitation extremes for the 5 stations in the Netherlands separately instead of bundling the station data to enhance the chance on trend detection. %in order to receive higher sample size .
