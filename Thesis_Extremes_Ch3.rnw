...
Note, in this chapter we include duration data to find the true precipitation intensity (PI). This allows us even to distinguish between hours with the same precipitation sum, of which one is very intense and short-lasting, and the other less intense and long-lasting. Here, P is the hourly mean precipitation intensity.
...
M <- hrKNMI[P>0][, list(P=P, I=I,T, TD, RH=RH,FH=FH)]
# create data table to study relation max P/I with max in other variables
M_max <- hrKNMI[, list(Pmax=max(P), Imax=max(I), Tmax=max(T), TDmax=max(TD),RHmax=max(RH),FHmax=max(FH),DD_maxP=DD[which.max(P)],WW_maxP=WW[which.max(P)]),by=list(Date,Season)]
...
allmax2d <- CAPE2d[, list(Pmax2d=max(P), Imax2d=max(I),Tmax2d=max(T),TDmax2d=max(TD),RHmax2d=max(RH),FHmax2d=max(FH),DD_maxP=DD[which.max(P)],WW_maxP=WW[which.max(P)]),by=d2]
...
save(Allmax2d, file="./inst/tussenStap/Allmax2d.rda")
Allmax2d_mat <- subset(allmax2d, select=c(Pmax2d, Imax2d, Tmax2d,TDmax2d,CAPEmax2d,RHmax2d,FHmax2d))
@
...
CAPE_day <- Compare[N>0][(Year > 1993) | (Month > 2)][, list(CAPEmax = max(sbCAPE, na.rm = TRUE), Psum=sum(P), Imax=max(I), Pmax=max(P),Tmax=max(T), N=mean(N)), by=Date]
save(CAPE_day,file="./inst/tussenStap/CAPE_day.rda")
@

<<Cor_CAPE>>=
load("./inst/tussenStap/CAPE_day.rda")
CAPE <- subset(CAPE_day,select=c(CAPEmax,Tmax, Psum, Pmax, Imax, N))
...
Cmat <- as.matrix(CAPE_corr[2:6,1])
dimnames(Cmat) <- list(c("Tmax","Psum","Pmax","Imax","N"),c("CAPEmax"))
@

To have an easy and clear overview of the strength of possible relationships between multiple variables, we plotted heatmaps of the Pearson correlation coefficients. The Pearson correlation coefficient is widely used statistics giving a measure of the strength of a linear relationship \citep{Zou2003}. The heatmaps show all possible relationship between the following 7 variables: PI, I, T, Td, FH (10m-wind strength, meaned as last 10 minutes of an hour), RH (relative humidity) and CAPE. For all variables, except for CAPE, we combined the measurements of the five stations, in order to clarify the relationships. Next to comparison of all hours, we also looked at the correlation coefficients for the 2-day maxima. For CAPE only measurements of station De Bilt are available (REF CH method). Next to a different spatial resolution, CAPE also has a different temporal resolution. Therefore, we plotted the relationships with CAPE in a seperate heatmap. As CAPE is sometimes measured once a day, is influenced by precipitation (REF CH TB?) and can differ a lot on a day, we compared it in two different ways with precipitation; (i) to the cumulative precipitation sum in the hours following a CAPE measurements, (ii) the 2-day maximum in CAPE. We take a 2-day maxima,
(maybe include duration of precipitation within a hour?)
...
@
    \subcaption{All variables, except CAPE, for all hours}
...
@
    \subcaption{2-day maxima for all variables}
...
Explain Pearson correlations, all wet hours
Explain 2-day maxima (look at extremes)

To see whether we get a stronger signal if we look at daily maxima, two times higher resolution, but lower extremes..
...
However, what if non-linear relationships???
Plot ...
...
Then talk about assumptions ... ; we saw in CH2 non-normal distribution of P and I ,
while we need two normal distributed variables to not violate assumption for Pearson.

Therefore, use of Spearman

\begin{figure}[H]
  \centering
  \textbf{Spearman Heat maps}\par\medskip
  \begin{minipage}{0.48\textwidth}
<<SPHeatmap_all>>=
# All hours
ggcorrplot(sp_corr, hc.order = FALSE,type = "lower",lab=TRUE)
@
    \subcaption{Spearman: All hours}
...
quantile regression time of normal and 2day max in time

Plot distribution
--> normal? LS fitting (T and TD)
--> lognormal/not-normal --> quantile regression (CAPE, FH?)
...
max2d_all <- data2d[, list(Pmax2d=max(P),Imax2d=max(I),Tmax2d=max(T),TDmax2d=max(TD),RHmax2d=max(RH),FHmax2d=max(FH),DD_maxP=DD[which.max(P)],WW_maxP=WW[which.max(P)]),by= list(d2,STN)]

allmax2d <- CAPE2d[, list(),by=d2]
@
...
M_max <- hrKNMI[, list(Pmax=max(P), Imax=max(I), Tmax=max(T), TDmax=max(TD),RHmax=max(RH),FHmax=max(FH)),by=list(Date,Season)]
...
<<PmaxvsTmean>>=
#Subset for maxima data
M_max <- hrKNMI[, list(Pmax=max(P), Imax=max(I), Tmean=mean(T), TDmax=max(TD),RHmax=max(RH),FHmax=max(FH)),by=list(Date,Season)]
#M_max[Pmax>0] #only wet days
# TEMPERATURE
...
CAPE_day <- Compare[N>0][(Year > 1993) | (Month > 2)][, list(CAPEmax = max(sbCAPE, na.rm = TRUE),Psum=sum(P), Pmax=max(P),Imax=max(I), N=mean(N)),by=list(Date,Season)]
