...
@
    ...
    \label{fig:CAPE_p}
  \end{minipage}
\caption{Heatmap consisting of Pearson correlation coefficients of the variables: hourly mean precipitation intensity (P), precipitation intensity (PI), wind strength (FH), relative humidity (RH), temperature(T) and dewpoint temperature(TD). The heatmaps are computed from: 2-day maxima of all variables (meaned over all stations), except CAPE (a), 2-day CAPE maxima in relation to other variables for de Bilt (b), and daily maxima of all variables except CAPE (c), daily CAPE maxima in relation to other variables for de Bilt (d). $N_d2$ (N) is the 2-day (daily) count of CAPE measurements. Colouring from blue to red gives the sign and strength of the relationship as is shown in the legend.}
...
%' @
%'     \subcaption{Daily Maxima}
%'     \label{fig:Heatmap_maxisp}
%'   \end{minipage}
  \caption{Heatmap consisting of Spearman correlation coefficients of the variables: intensity (I), wind strength (FH), measure of relative humidity (RH), hourly precipitation sum (P), temperature(T) and dewpoint temperature(TD).The heatmaps are computed from: 2-day maxima of all variables (meaned over all stations), except CAPE (a), 2-day CAPE maxima in relation to other variables for de Bilt (b). N_d2 is the 2-day count of CAPE measurements. Colouring from blue to red gives the sign and strength of the relationship as is shown in the legend.}
  \label{fig:Heatmap_max2dsp}
\end{figure}

The Spearman correlation coefficients are all lower than the Pearson correlation coefficients for wet 2-day maxima, except for those corresponding to the possible relations with FH 2-day maxima, the relations between the different precipitation maxima (P, PI and Psum) and the linear relation between T and TD 2-day maxima. %Whether these relationships are found linear is based on \ref{fig:pointsclouds_2}
The wind speed is not only for 2-day precipitation sums, but also for 2-day maxima in P and PI the strongest explanatory factor (coefficients of 0.37-0.47).
It's strange that although the relationships mainly seem to be non-linear that the Spearman correlation coefficients are not higher in general, than the Pearson correlations. However, the Spearman approach is more correct from a statistical point of view.

To summarize, ..... have high correlation..

%' \begin{figure}[H]
%'   \centering
%'   \textbf{Point clouds of all wet hours}\par\medskip
%' <<Plotting_points_allvariables2,cache=TRUE>>=
%' #M <- hrKNMI[P>0][, list(P=P, PI=I,DR=DR,T, TD, RH=RH,FH=FH)]
%' M <- subset(M, select=c("T","TD","P","PI","DR")) #, "PImax"
%' M_wet <- M[P>0]
%' plot(M_wet)
%' @
%'   \caption{Points clouds showing the type of relationship between the maxima of the following variables: temperature(T), dewpoint temperature(TD), hourly mean precipitation intensity (P), hourly intensity (PI) and precipitation duration within an hour (DR). Note, the x- and y-axes differ in scale.}
%'   \label{fig:pointclouds_2}
%' \end{figure}
...
% Mind mean (max) over station or bundle with vectors
...
plot(density(Allmax2d_mat$CAPEmax2d),col="darkgreen",lwd=3, main=NA,   #Distribution of nr of wet hours
...
#   geom_point(aes(y=max),color="gray",size=0.3) +
#   #geom_smooth(aes(y=max), color="black") +
...
#   ggtitle(title) + # title
#   #ylim(0,ymax) +
#   #scale_x_discrete(limits=CAPEmax2d$Year)+
#   scale_x_continuous(breaks=c(366,1279,2193,3106,4019),labels=c(1995,2000,2005,2010,2015)) +
#   xlab("Time (2 days)") + ylab("CAPE max (J/kg)")+ # x and y-axis label
#   theme_bw()


QR_max2dplotting(data=QR_CAPEmax2d, title="CAPE",ytitle="CAPE max (J/kg)",LM=FALSE)
# As CAPE in winter is < 500 J/kg, trends are visible for both seasons, both stronger for summer
@
...
FH <- data2d[!is.na(FH)][, list(FHmax2d=max(FH),DD_maxP=DD[which.max(P)],Year=round(mean(Year),digits=0)),by=d2] #list(d2,STN)
FHmax2d <- subset(FH, select=c("d2","Year","FHmax2d"))
setnames(FHmax2d,c("d2","Year","max"))
FHmax2d[,yr_mean := mean(max),by=Year]
save(FHmax2d,file="./inst/tussenStap/FHmax2d.rda")

# Check!
# 10592 d2's (* 5 stns = 52960)     Is correct, namely nr of d2's: 365.25/2*58 = [1] 10592.25
# Double check
#Check amount of 2days a year FHmax2d[,length(d2),by=Year] --> seems good
# FHmax2d[duplicated(d2),] --> 0  , Empty data.table (0 rows) of 15 cols:

QR_FHmax2d <- QR_alld2(FHmax2d)
# Plotting
QR_max2dplotting(data=QR_FHmax2d, title="FH",ytitle="FH max (m/s)")
@

<<Slopes_FH>>=
qs <- c(0.25,0.50,0.75,0.90,0.95,0.99)
FHqr <- rq(max ~ d2, data=FHmax2d, tau = qs)
FHqr_yearly <- rq(max ~ Year, data=FHmax2d, tau = qs)
#summary(FHqr)
...
# Comparison periods 1958-1993, 1993-2015, not so big difference in the end
# Different resolution after 1993; AWS
# Try for different periods
#FHmax2d_begin <- FHmax2d[Year<1993]
#FHmax2d_end <- FHmax2d[Year<1994]
###
# qs <- c(0.25,0.50,0.75,0.90,0.95,0.99)
# FHqr <- rq(max ~ d2, data=FHmax2d_begin, tau = qs)
# FHqr_yearly <- rq(max ~ Year, data=FHmax2d_begin, tau = qs)
# #summary(FHqr)
# plot(summary(FHqr), parm="d2")
# mtext("Quantiles",side=1,line=4)
# mtext("Slopes (J/kg/2 days)",side=2,line=3)
...
# mtext("Slopes (J/kg/yr)",side=2,line=3)
#
# Begin: -0.06-0 J/kg/yr , LM of approx -0.05
###
# qs <- c(0.25,0.50,0.75,0.90,0.95,0.99)
# FHqr <- rq(max ~ d2, data=FHmax2d_end, tau = qs)
# FHqr_yearly <- rq(max ~ Year, data=FHmax2d_end, tau = qs)
# #summary(FHqr)
# plot(summary(FHqr), parm="d2")
# mtext("Quantiles",side=1,line=4)
# mtext("Slopes (J/kg/2 days)",side=2,line=3)
...
# mtext("Slopes (J/kg/yr)",side=2,line=3)
#
# End: -0.05-0.04 J/kg/yr , LM of approx -0.05
@
...
Tmax2d <- Tallstn[,max := mean(Tmax2d),by=d2]
...
Tmax2d <- Tmax2d[,list(Year=unique(Year),max=unique(max)),by=d2]
Tmax2d[,yr_mean := mean(max),by=Year]
save(Tmax2d,file="./inst/tussenStap/Tmax2d.rda")
#Tmax2d is per station, max is meaned for all stations!
...
# Plotting quantile regression lines of mean T max of all stations
QR_Tmax2d <- QR_alld2(Tmax2d)
QR_max2dplotting(data=QR_Tmax2d, title="T",ytitle="T max (m/s)")
# Plotting QR of max T of all stations
QR_Tmax2d_allstn <- QR_alld2(Tmax2d_allstn)
QR_max2dplotting(data=QR_Tmax2d_allstn, title="T",ytitle="T max (m/s)")
...
<<Slopes_T,fig.keep='all'>>=
Tqr <- rq(max ~ d2, data=Tmax2d, tau = qs)
Tqr_yearly <- rq(max ~ Year, data=Tmax2d, tau = qs)
#summary(Tqr)
...
<<QR_TD>>=
TD <- data2d[, list(TDmax2d=max(TD),Year=round(mean(Year),digits=0)),by=d2] #list(d2,STN)
TDmax2d <- subset(TD, select=c("d2","Year","TDmax2d"))
setnames(TDmax2d,c("d2","Year","max"))
TDmax2d[,yr_mean := mean(max),by=Year]
save(TDmax2d,file="./inst/tussenStap/TDmax2d.rda")
# Checked! (similar to others)

QR_TDmax2d <- QR_alld2(TDmax2d)
# Plotting
QR_max2dplotting(data=QR_TDmax2d, title="TD",ytitle="TD max (m/s)")
@

<<Slopes_TD,fig.keep='all'>>=
TDqr <- rq(max ~ d2, data=TDmax2d, tau = qs)
TDqr_yearly <- rq(max ~ Year, data=TDmax2d, tau = qs)
#summary(TDqr)
