...
@
  ...
  \label{fig:Spearsonheatmap}
\end{figure}

The Spearman correlation coefficients are all lower than the Pearson correlation coefficients for wet 2-day maxima, except for those corresponding to the possible relations with FH 2-day maxima, the relations between the different precipitation maxima (P, PI and Psum) and the linear relation between T and TD 2-day maxima (Figure \ref{Spearsonheatmap}). %Whether these relationships are found linear is based on \ref{fig:pointsclouds_2}
...
Note, trend analysis on 2-day CAPE maxima is applied for the time period for which CAPE data is available (1993-2015) and not meaned over stations, as only CAPE data from de Bilt is available. We do not study in depth the historical changes in instrumentation and measuring sites as we did for precipitation, due to the time restriction of this research. Third, we apply a significance test on the trends.
...
In order to investigate this, and also for the trends in the 2-day maxima of the other variables, we conduct a 9999 Monte Carlo permutation test. Table \ref{p_rates_all} gives an overview of the significance of the trends. All the linear and quantile regression trends are significant, except for the 99\%-quantile of 2-day maxima in FH (p=0.94), of which we already discusses the large uncertainty expressed by the wide confidence interval for this quantile. In fact, all the trends are significant on the 99\% confidencence level, except for the 99\%-quantile for FH and CAPE. It should be stressed that the 2-day maxima in CAPE are based only on data of De Bilt and for the period 1993-2015. Due to this different temporal lenght of the data set the resolution is 60\% lower than for the 2-day maximain T, TD and FH. Nontheless, the 99\%-quantile trend for this variable is significant on the 95\% confidence level and the other quantile trends on the 99\% confidence level.
...
(Maybe add summer and winter plots of quantile rates+ p table??)
...
For the most important factors (maxima in T, TD and CAPE) regarding the correlation heat maps (\ref{subsubsec:correlationmatrices}) we plotted the relationship between the daily maximum hourly precipitation sums and the maxima of other variables as point clouds. --> DOUBLE??

As the type of relationship can be influenced by how the data is distributed, we also plotted the distribution. We differentiated between summer and winter to have a better understanding of seasonal differences in precipitation explained by the key variables.
...
% Improve, but use bundled data!
<<FH,eval=FALSE>>=
#2 day max + d2 + year of FH, T, TD
load("./inst/tussenStap/data2d.rda")
# all variables except CAPE --> as it has other resolution
FH <- data2d[!is.na(FH)][, list(FHmax2d=max(FH),DD_maxP=DD[which.max(P)],Year=round(mean(Year),digits=0)),by=list(d2,STN)]
save(FH,file="./inst/tussenStap/FH.rda")

# Seasons
load(file="./inst/tussenStap/data2dS.rda")
max2dS <- data2dS[, list(Pmax2d=unique(Pmax2d), PImax2d=max(I),Psum2d=sum(P),DR_maxP=DR[which.max(P)],Tmax2d=max(T),T_max2dP= T[which.max(P)],TDmax2d=max(TD),TD_max2dP= TD[which.max(P)],RHmax2d=max(RH),FHmax2d=max(FH),FH_max2dP= FH[which.max(P)],DD_maxP=DD[which.max(P)],WW_maxP=WW[which.max(P)],Year=round(mean(Year),digits=0)),by=list(d2,STN)]
save(max2dS, file="./inst/tussenStap/max2dS.rda")
FHS <- subset(max2dS[!is.na(FH)], select=c("d2","FHmax2d","Year"))
save(FHS,file="./inst/tussenStap/FHS.rda")

load(file="./inst/tussenStap/data2dW.rda")
max2dW <- data2dW[, list(Pmax2d=unique(Pmax2d), PImax2d=max(I),Psum2d=sum(P),DR_maxP=DR[which.max(P)],Tmax2d=max(T),T_max2dP= T[which.max(P)],TDmax2d=max(TD),TD_max2dP= TD[which.max(P)],RHmax2d=max(RH),FHmax2d=max(FH),FH_max2dP= FH[which.max(P)],DD_maxP=DD[which.max(P)],WW_maxP=WW[which.max(P)],Year=round(mean(Year),digits=0)),by=list(d2,STN)]
save(max2dW, file="./inst/tussenStap/max2dW.rda")
FHW <- subset(max2dW[!is.na(FH)], select=c("d2","FHmax2d","Year"))
save(FHW,file="./inst/tussenStap/FHW.rda")

FHtrial <- rq(FHmax2d ~ d2, data=FH[STN==260], tau=0.90)

data <- FH[, f95 := as.numeric(fitted.values(rq(FHmax2d ~ d2, tau =0.95,method="fn")))]
data <- FH[STN==235][, f95 := as.numeric(fitted.values(rq(FHmax2d ~ d2, tau =0.95,method="fn")))]
data <- FH[STN==260][, f95 := as.numeric(fitted.values(rq(FHmax2d ~ d2, tau =0.95,method="fn")))]
data <- FH[STN==280][, f95 := as.numeric(fitted.values(rq(FHmax2d ~ d2, tau =0.95,method="fn")))]
data <- FH[STN==310][, f95 := as.numeric(fitted.values(rq(FHmax2d ~ d2, tau =0.95,method="fn")))]
data <- FH[STN==380][, f95 := as.numeric(fitted.values(rq(FHmax2d ~ d2, tau =0.95,method="fn")))]
