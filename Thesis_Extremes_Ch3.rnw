...
# relation max P/I with max in other variables
...
M_max <- hrKNMI[P>0][, list(Pmax=max(P), Imax=max(I), Tmax=max(T), TDmax=max(TD),RHmax=max(RH),FHmax=max(FH)),by=list(Date,Season)]
...
CAPE_day <- Compare[N>0][(HH==11)|(HH=17)][(Year > 1993) | (Month > 2)][, list(CAPEmax = max(sbCAPE, na.rm = TRUE),Psum=sum(P), Pmax=max(P),Imax=max(I), N=mean(N)),by=list(Date,Season)]
...
<<lagTD>>=
lagTD <- hrKNMI[P>0][, list(Pmax=max(P),TDmax=max(TD),TD_HH=HH[which.max(TD)],P_HH=HH[which.max(P)]),by=list(Date,Season)]
