...
max2d <- CAPE2d[, list(Pmax2d=max(P), Tmax2d=max(T),TDmax2d=max(TD),CAPEmax2d=max(sbCAPE),max2d=max(max2d),Year=unique(Year),YM=unique(YM),month=unique(month)),by=d2]
max2d[,yr_Pmean := mean(Pmax2d),by=Year]
