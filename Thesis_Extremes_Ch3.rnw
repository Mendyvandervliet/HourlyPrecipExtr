...
. Besides, not only the 90\%- and 95\%-quantile lines have a stronger slope than the OLS line, but also the 25\%- and 50\%-quantile lines. The OLS of the yearly means T maxima gives an increase of 3.4 $^{\circ}$C/century, which is almost four times as high as the global rate in temperature for this time period (\ref{subsec:theory}). % 0.034 / 12.75 * 100 = 0.27%/yr = for 58 yrs 15.5%
Note that we only consider heavy hourly precipitation (i.e. 2-day maxima of precipitation) measured in the Netherlands, and we know from Chapter (INTRO REF) that local extremes increase faster than the global average.\\
% COMPARE relative trends to relative trends in 2-day P, F and Psum
<<QR_TD>>=
