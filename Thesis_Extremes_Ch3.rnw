...
\subsubsection{Specific or Relative humidity}
...
M_max <- hrKNMI[RH>0][, list(Pmax=max(RH), Imax=max(I), Tmax=max(T), TDmax=max(TD),RHmax=max(U),FHmax=max(FH),DD_maxP=DD[which.max(RH)]),by=list(Date,Season)]
...
sbCAPE_check <- CAPE[sbCAPE<5000][, list(sbCAPE,Dat)]
...
# 1) Maxima
CAPE_day <- Compare[N>0][(Year > 1993) | (Month > 2)][, list(CAPEmax = max(sbCAPE, na.rm = TRUE), RHsum=sum(RH), maxI=max(I), Tmax=max(T), N=mean(N)), by=Date]
...
Q_Tbin <- Q_Tbin[,list(Pmeanmax=mean(Pmax),freq=.N,Q95=quantile(Pmax,probs=0.95),Q99=quantile(Pmax,probs=0.99)),by=list(T_bin,Season)]
...
Q_Tbin[,Tmid := c(seq(6,12,by=2),seq(12,20,by=2))]
# Determine intercept for Clausius Clapeyron Exponential relations
StartCC <- Q_Tbin[Tmid==6]$Pmeanmax
# Calculate CC-lines
Q_Tbin[,":="(CC = Pmeanmax[1]*(1.07^(Tmid-Tmid[1])), CC95 = Q95[1]*(1.07^(Tmid-Tmid[1])),CC99 = Q99[1]*(1.07^(Tmid-Tmid[1])))]
@

<<Plot_PmaxvsTmax,fig.cap='The maxima in Precipitation are plotted against maxima in temperature (A). B shows the distribution of temperature maxima, and C demonstrates mean and the 95-percent quantile change in precipitation for every bin temperature. Temperature is binned with steps of 2 degrees.'>>=
# Plot point cloud Pmax vs Tmax
plot1 <- ggplot(M_Tmax, aes(x=Tmax)) + geom_point(aes(y=Pmax,color=Season))
  # scale_colour_manual(values = c("#CC6600","#009966","#FF0000","darkblue"),breaks=c("Winter","Spring","Summer","Autumn"))
# Plot histogram Tmax
plot2 <- ggplot(M_Tmax) + geom_histogram(aes(x=Tmax,fill=Season),position="dodge",binwidth=2)
...
plot3 <- ggplot(Q_Tbin,aes(x=Tmid)) + geom_line(aes(y=Pmeanmax,color=Season), linetype=1)+
  ...
  geom_line(aes(y=CC)) + geom_line(aes(y=CC95),linetype=2) + geom_line(aes(y=CC99),linetype=3) +xlab("Tmax") + ylab("Pmax")

plot_grid(plot_grid(plot1, plot2, plot3, labels=c("A", "B","C"), ncol = 1, nrow = 3))
...
Point cloud -->   ,
As the number of measurement per degree temperature differs and this might explain the exponential relation, we also plotted the mean precipitation maxima per bin. In order to include the \enquote{high} extremes we also plotted the 95\% and 99\% quantile lines.

Clausius clapeyron

can't be explained by winter --> summer difference in precipitation maxima or ,
but other factors may play a role (convective vs stratiform)
...
plot3 <- ggplot(Q_TDbin,aes(x=TDmid)) + geom_line(aes(y=Pmeanmax,color=Season), linetype=1)+
  geom_line(aes(y=Q95,color=Season),linetype=2) + geom_line(aes(y=Q99,color=Season),linetype=3) +
  geom_line(aes(y=CC)) + geom_line(aes(y=CC95),linetype=2) + geom_line(aes(y=CC99),linetype=3) +xlab("TDmax") + ylab("Pmax")
...
CAPE_day <- Compare[N>0][(HH==11)|(HH=17)][(Year > 1993) | (Month > 2)][, list(CAPEmax = max(sbCAPE, na.rm = TRUE),
                                                     RHsum=sum(RH), Pmax=max(RH),Imax=max(I), N=mean(N)),by=list(Date,Season)]
...
@
`
<<Plot_PmaxvsCAPEmax,fig.cap='The maxima in Precipitation are plotted against CAPE values (A), B shows the distribution of CAPE maxima, and C demonstrates mean and the 95-percent quantile change in precipitation for every CAPE. CAPE is binned with steps of 50 J kg$^{-1}$.'>>=
# Plot point cloud Pmax vs CAPEmax
plot1 <- ggplot(M_CAPEmax, aes(x=CAPEmax)) + geom_point(aes(y=Pmax,color=Season))+facet_wrap(~Season) + scale_x_continuous(breaks=c(0,1000,2000))
# scale_colour_manual(values = c("#CC6600","#009966","#FF0000","darkblue"),breaks=c("Winter","Spring","Summer","Autumn"))
# Plot histogram CAPEmax
plot2 <- ggplot(M_CAPEmax)+ geom_histogram(aes(x=CAPEmax,fill=Season),binwidth=50,position="dodge")
...
plot3 <- ggplot(Q_CAPEbin,aes(x=CAPEmeanmax)) + geom_line(aes(y=Pmeanmax,color=Season), linetype=1)+
  geom_line(aes(y=Q95,color=Season),linetype=2)  + ylim(0,8) +
  xlab("CAPEmax") + ylab("Pmax")
...
Q_TDbinW[,":="(CC1 = 0.46*exp(0.07*TDmid), CC2 = Pmeanmax[1]*exp(0.07*TDmid), CC95 = Q95[1]*exp(0.07*TDmid),CC99 = Q99[1]*exp(0.07*TDmid))]
Q_TDbinW <- within(Q_TDbinW, Q95[freq<100] <- NA)
Q_TDbinW <- within(Q_TDbinW, Q99[freq<100] <- NA)
# Calculate CC-lines
# P = A exp(1.07T)
# At T=0, P=A.  A=
M_allmax[TDmax==0][,mean(Pmax)]
A = 0.46
# In our case we start at Tmid=5, we take A=P0
# Q_TDbinW[,":="(CC = Pmeanmax[1]*(1.07^(TDmid-TDmid[1])), CC95 = Q95[1]*(1.07^(TDmid-TDmid[1])),CC99 = Q99[1]*(1.07^(TDmid-TDmid[1])))]
@

<<Wind_TDplotting>>=
# Plot point cloud Pmax vs TDmax
plot1 <- ggplot(M_allmax, aes(x=TDmax)) + geom_point(aes(y=Pmax,color=W_maxP)) + facet_wrap(~W_maxP)
# scale_colour_manual(values = c("#CC6600","#009966","#FF0000","darkblue"),breaks=c("Winter","Spring","Summer","Autumn"))
# Plot histogram TDmax
plot2 <- ggplot(M_allmax) + geom_histogram(aes(x=TDmax,fill=W_maxP),position="dodge",binwidth=2) #+ facet_wrap(~W_maxP)
# Plotted Pmax binned over TDmax (2 degrees bin)
plot3 <- ggplot(Q_TDbinW,aes(x=TDmid)) + geom_line(aes(y=Pmeanmax,color=W_maxP), linetype=1) + #geom_line(aes(y=CC1)) +
 geom_line(aes(y=Q95,color=W_maxP),linetype=2) + geom_line(aes(y=Q99,color=W_maxP),linetype=3) + geom_line(aes(y=CC2)) + geom_line(aes(y=CC95),linetype=2) + geom_line(aes(y=CC99),linetype=3) +xlab("TDmax") + ylab("Pmax") + scale_y_log10()  + facet_wrap(~W_maxP)
...
DT_bin <- M_allmax[(TDmax>-0.1) &(TDmax<18.1)][CAPEmax<2500][,":="(TD_bin = .bincode(TDmax, breaks=seq(0,18,0.2),include.lowest=TRUE),CAPE_bin = .bincode(CAPEmax, breaks=seq(0,2500,25),include.lowest=TRUE))]
...
# Show only summer and winter season, and for bins with N, number of measurements, higher than 100
DT_bin <- DT_bin[(Season=="Summer")|(Season=="Winter")][order(CAPE_bin)][freq>10]

#g <- expand.grid(x=DT_bin$CAPEmeanmax ,y = DT_bin$TDmeanmax)

wireframe(Pmeanmax ~ CAPEmeanmax*TDmeanmax,data=DT_bin,groups=Season,scales = list(arrows = FALSE),drape= TRUE,aspect= c(2,1),colorkey= TRUE)
# --> out of bound
