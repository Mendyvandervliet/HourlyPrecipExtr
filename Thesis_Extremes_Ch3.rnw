...
allmax2d <- CAPE2d[, list(Pmax2d=max(P), PImax2d=max(I),DR_maxP=DR[which.max(P)],Tmax2d=max(T),T_max2dP= T[which.max(P)],TDmax2d=max(TD),TD_max2dP= TD[which.max(P)],RHmax2d=max(RH),FHmax2d=max(FH),FH_max2dP= FH[which.max(P)],DD_maxP=DD[which.max(P)],WW_maxP=WW[which.max(P)],Year=round(mean(Year),digits=0)),by=d2]
...
@
    \subcaption{Wet daily maxima for all variables}
    \label{fig:Heatmap_maxip}
  \end{minipage}
\caption{Heatmap consisting of Pearson correlation coefficients of the variables: hourly mean precipitation intensity (P), precipitation intensity (PI), wind strength (FH), relative humidity (RH), temperature(T) and dewpoint temperature(TD). The heatmaps are computed from: all wet hours for all variables except CAPE (a), from daily CAPE maxima in relation to other variables (b), for 2-day maxima of all variables (c) and daily maxima of all variables except CAPE (d). Colouring from blue to red gives the sign and strength of the relationship as is shown in the legend.}
...
@
  ...
  \end{minipage}
  \caption{Heatmaps consisting of Spearman correlation coefficients of the variables: intensity (I), wind strength (FH), measure of relative humidity (RH), hourly precipitation sum (P), temperature(T) and dewpoint temperature(TD). The heatmaps are computed from: all wet hours for all variables except CAPE (a), from daily CAPE maxima in relation to other variables (b), for 2-day maxima of all variables (c) and daily maxima of all variables except CAPE (d). Colouring from blue to red gives the sign and strength of the relationship as is shown in the legend.}
  \label{fig:Spearmanheatmap}
\end{figure}

The Spearman correlation coefficients are all lower than the Pearson correlation coefficients for all wet hours, except for those corresponding to the relation between duration and TD or T, and to the linear relation between T and TD. %Whether these relationships are found linear is based on \ref{fig:pointsclouds_2}
Whereas the correlation coefficient between P and PI decreases slightly, the correlation coefficient between DR and P increases notable (from 0.35 to 0.59). The correlation coefficients between PI or P and the explanatory factors T, TD, and FH, all decrease a bit. Is unexpected, as these are non-linear relationships

However, 2-day maxima of Spearman are higher?
Maybe show only Spearman 2-day, as this gives extremes better than all, and daily does not improve significantly , so better comparable to found trends!
