...
M <- hrKNMI[RH>0][, list(P=RH, I=I,T, TD, RH=U)]
M_max <- hrKNMI[RH>0][, list(Pmax=max(RH), Imax=max(I), Tmax=max(T), TDmax=max(TD),RHmax=max(RH)),by=Date]
M_maxi <- subset(M_max, select=c(Pmax, Imax, Tmax, TDmax,RHmax))
