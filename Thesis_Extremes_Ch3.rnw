...
As already explained in \ref{subsubsec:T} an increase in the present and/or strength of the updrafts, which is characterized by the degree of CAPE, can lead to enhanced moisture influx.

INCLUDE SENTENCE ABOUT relation CAPE with extreme precipitation
...
\subsubsection{Wind strenght}

ADD: Wind speed is the result of a horizontal temperature-related pressure gradient and CAPE-associated convergence of air.  WITH SOURCE

Severe local storms are associated with pronounced shifts in wind speeds and/or direction with height: wind shear \citep{Browning1964,Wingo2010}. The four types of storms distinguished on the way of development are: supercell, multicell, a collection of numerous individual precipitation cells as f.e. squall-line storms and mesoscale convective complexes \citep[p. 156]{Sumner1988}. Some of these storms can develop into larger storms delivering an hour or two of very intense precipitation at a point. A storm can obtain a self-perpetuating character due to pronounced wind shear, as this permits the continual introduction of fresh and moist air into the system at the lowel-level developing edge and the venting of used air aloft, while the downdraught remains separated from the updraughts \citep[p. 156]{Sumner1988}. Wind shear is therefore related to the lifetime of a storm (up to several hours). (Side remark; here the use of the word storm, better in context?)
Unfortunately we only have hourly observations of wind speed at higher levels for our time period of 58 years. Instead, we can study the horizontal wind speed at 10 m, and use this a partial indicator of wind shear. Higher wind speeds also lead to faster propagation of fronts and local showers over a station. Therefore, we would expect a higher intensity (precipitation sum over duration of precipitation within an hour), and to a lower extent a higher hourly mean intensity (precipitation sum over an hour. In other words, changes in wind speed could influence the intensity of precipitation in two ways; (i) via the organisation of a storm under restriction of strong wind shear, and (ii) via faster propagation of precipitation systems over a station.
...
(maybe add summarizing alinea with link to next section?)

\subsection{Identification of relationships}

In this part we try to identify relationships explaining the trends found in hourly precipitation in Chapter 2 (REF CH2). First, we start with a study of correlations for the variables described as important to precipitation intensity in Section \ref{subsec:theory}. Next, we compare trends in the key variable(s) to the observed trends in hourly intensity. Lastly, the type of relationship (e.g. linear or non-linear) is studied, thereby we account for the influence of how the data is distributed. %Our analysis is followed by plotting and interpreting lags. To find the best model for precipitation data the most promising variables are combined in multiregression plots.
(Side remark: misschien dubbelop? Ik heb in het begin van het hoofdstuk al de indeling verteld ) Note, in this chapter we include duration, defined as the hourly time fraction of precipitation, data to find the true precipitation intensity (PI). This allows us even to distinguish between hours with the same precipitation sum, of which one is very intense and short-lasting, and the other less intense and long-lasting. Here, P is the hourly precipitation sum.
...
M_max <- hrKNMI[, list(Pmax=max(P), PImax=max(I),Psum=sum(P),DR_maxP=DR[which.max(P)], Tmax=max(T), T_maxP=T[which.max(P)],TDmax=max(TD),TD_maxP=TD[which.max(P)],RHmax=max(RH),FHmax=max(FH),FH_maxP=FH[which.max(P)],DD_maxP=DD[which.max(P)],WW_maxP=WW[which.max(P)]),by=list(Date,Season)]
...
M_maxi <- subset(M_max[!is.na(FHmax)], select=c(Pmax, PImax,Psum,Tmax,TDmax,RHmax,FHmax))#,DR_maxP
...
CAPE2dmax <- CAPE2d[!is.na(sbCAPE)][,list(CAPEmax2d=max(sbCAPE),N_d2=sum(N)),by=d2] # Remove d2's with missing CAPE values
save(CAPE2dmax,file="./inst/tussenStap/CAPE2dmax.rda")

load("./inst/tussenStap/CAPE2dmax.rda")

# 2-day maxima per station
allmax2d <- data2d[, list(Pmax2d=max(P), PImax2d=max(I),Psum2d=sum(P),DR_maxP=DR[which.max(P)],Tmax2d=max(T),T_max2dP= T[which.max(P)],TDmax2d=max(TD),TD_max2dP= TD[which.max(P)],RHmax2d=max(RH),FHmax2d=max(FH),FH_max2dP= FH[which.max(P)],DD_maxP=DD[which.max(P)],WW_maxP=WW[which.max(P)],Year=round(mean(Year),digits=0)),by=list(d2,STN)]
save(allmax2d, file="./inst/tussenStap/allmax2d.rda")

# 2-day maxima, meaned over all stations
allmax2d_mean <- allmax2d[, list(Pmax2d=mean(Pmax2d), PImax2d=mean(PImax2d),Psum2d=mean(Psum2d),DR_maxP=mean(DR_maxP),Tmax2d=mean(Tmax2d),T_max2dP= mean(T_max2dP),TDmax2d=mean(TDmax2d),TD_max2dP=mean(TD_max2dP),RHmax2d=mean(RHmax2d),FHmax2d=mean(FHmax2d),FH_max2dP=mean(FH_max2dP),DD_maxP=mean(DD_maxP),WW_maxP=mean(WW_maxP),Year=unique(Year)),by=d2]
save(allmax2d_mean, file="./inst/tussenStap/allmax2d.rda")
#Allmax2d <-merge(CAPE2dmax,allmax2d_mean,by="d2")

# Select variables of interest and remove missing values (present in FHmax2d)
allmax2d_mat <- subset(allmax2d_mean[!is.na(FHmax2d)], select=c(Pmax2d, PImax2d,Psum2d,Tmax2d,TDmax2d,RHmax2d,FHmax2d)) #,DR_maxP
allmax2d_mat <- allmax2d_mat[Pmax2d>0]  # all wet 2-days
...
CAPEmax2d_mat <- subset(CAPEmax2d[d2>23], select=c(CAPEmax2d,Pmax2d, PImax2d, Psum2d,DR_maxP, Tmax2d,TDmax2d,RHmax2d,FHmax2d,N_d2)) #,DR_maxP
CAPEmax2d_mat <- CAPEmax2d_mat[Pmax2d>0]  # all wet 2-days
...
dimnames(Cmatp) <- list(c("Pmax","PImax","Psum","DR_maxP","Tmax","FHmax","N"),c("CAPEmax"))
save(Cmatp,file="./inst/tussenStap/Cmatp.rda")

Cmatsp <- as.matrix(CAPE_spcorr[2:8,1])
dimnames(Cmatsp) <- list(c("Pmax","PImax","Psum","DR_maxP","Tmax","FHmax","N"),c("CAPEmax"))
...
dimnames(CAPE_mat2d) <- list(c("Pmax2d","PImax2d","Psum2d","DR_maxP","Tmax2d","TDmax2d","RHmax2d","FHmax2d","N_d2"),c("CAPEmax2d"))
...
\begin{figure}[H]
  \centering
  \textbf{Heat maps of Pearson coefficients all wet hours}\par\medskip
<<PHeatmap_all,fig.height=4,fig.width=4>>=
# All hours
ggcorrplot(p_corr, hc.order = FALSE,type = "lower",lab=TRUE)
@
    \caption{Heatmap consisting of Pearson correlation coefficients for all possible relations, except those with CAPE, for all wet hours}
    \label{fig:Heatmap_allp}
\end{figure}

Figure \ref{fig:Heatmap_allp} provides an overview of the Pearson correlation coefficients for all variables, except CAPE, for all wet hours. The two highest coefficients are between T and TD (0.97) and P and I (0.75). The strength of these relationships is pretty comprehendable for wet hours.  For the first relationship, we know that .%

 The clear relationship between T and TD (dewpoint temperature) is straight forward; following the definition of dew point temperature itself: the dewpoint temperature is the temperature till which air has to be cooled to become saturated. For moist air (RH$>50$\%) the following thumb rule hold: $Td=T-(\frac{100-85}{5}$) \citep{Wallace2006}. As the data considered in this chapter only contains the wet hours, most of them will fulfill the assumption of a moist air and therefore a strong linear correlation is present. This corresponds to Pearson correlation coefficient of almost 1. The mean hourly precipitation intensity (PI) is the product of the hourly duration, DR, and intensity, I. The relationship between PI and I is stronger (0.75), than the relationship between PI and DR (0.35). % because I is computed from PI and DR
The third highest coefficient is the one between DR and RH (0.38). The higher the relative humidity, the higher the percentage is fullfilled which is needed for saturation at a certain temperature. Apparently this percentage says more about the duration of precipitation within an hour, than about the magnitude of the precipitation sum (or mean precipitation intensity PI). Higher percentage of RH correspond partly to longer durations of precipitation within an hour, which could mean that with higher saturation it takes longer before precipitation stops due to lowered saturation levels. However, there are of course also events where RH is high and the precipitation intensive, this explains why the coefficient is not super high.  % It is also explainable that this coefficient is not higher than 0.38, as there are also short-lastig, high intensity precipitation events.
Four other postive correlation coefficients, which are a bit higher than the rest, are those between (i) PI and T, (ii) PI and Td, (iii) I and T, and (iv) I and Td. From section \ref{subsec:theory} we know that temperature can influence precipitation amount (P) and intensity (PI) via enhanced evaporation and updrafts and higher ability of air to hold moisture. As evaporation is important at the place and time of moisture influx, it could be that this relation holds a non-insitu and lagged relationship. Hence, the relationship does not have to be significant for comparison between temperature and precipitation measured at the same time and only on the location of precipitation. The relationship between P or I and T via enhanced updrafts, should also be present in the relationship between CAPE and P or I. For De Bilt we plotted the CAPE 2-day maxima in relationship to other 2-day maxima and the cumulative 2-day precipitation sum (see Figure \ref{CAPE2d_p}).
...
@
    ...
    \label{fig:CAPE_p}
  \end{minipage}
\caption{Heatmap consisting of Pearson correlation coefficients of the variables: hourly mean precipitation intensity (P), precipitation intensity (PI), wind strength (FH), relative humidity (RH), temperature(T) and dewpoint temperature(TD). The heatmaps are computed from: 2-day maxima of all variables (meaned over all stations), except CAPE (a), 2-day CAPE maxima in relation to other variables for de Bilt (b), and daily maxima of all variables except CAPE (c), daily CAPE maxima in relation to other variables for de Bilt (d). $N_{d2}$ (N) is the 2-day (daily) count of CAPE measurements. Colouring from blue to red gives the sign and strength of the relationship as is shown in the legend.}
\label{fig:Pearsonheatmap}
\end{figure}

The relationship between CAPE and T is indeed relatively strong for this time scale and for this location, as can be deduced from the high correlation coefficient (0.35) in Figure \ref{fig:CAPE_p}.
In spite of all these relationships between T and P, the coefficient between all-hour P and T is not so high (0.15). However, more and counteracting relationships can play a role. What is more striking from Figure \ref{fig:Heatmap_allp} is the relatively high negative correlation between RH and FH. So the higher the wind speed, the lower the relative humidity. This could mean that with high wind speed at the moment of precipitation, the frontal or other precipitation system is blown over quickly. However, then DR is excepted to have a relatively high correlation with FH, which is not the case. On contrast, high horizontal wind speeds also occur at times of strong vertical mixing when contrasting winds converge at the place where air is lifted. Nontheless,this relationship between 2-day maxima in CAPE (CAPEmax2d) and FH is not found back in the corresponding correlation coefficient (-0.1) in Figure \ref{fig:CAPE2d_p}. The highest correlation between 2-day CAPE and precipitation variables, is the one between CAPEmax2d with the 2-day maxima in precipitation intensity (0.14). N shows the effect of the number of CAPE measurements a 2 days on the magnitude of the CAPE 2-day maxima, which is just as strong as the relation between CAPEmax2d and PImax2d, but weaker than the relation between 2-day CAPE and temperature or dewpoint temperature. As we studied the maxima in mean hourly precipitation intensity for independent 2-day periods in Chapter 3 (REF CH trends), we also included the correlation coefficients for 2-day maxima in P and I and the 2-day precipitation sum (Psum) in relation to the 2-day maxima in T, Td, FH, and RH. Eventually we want to explain the observed trends found in Chapter 3. Therefore, it is relevant to investigate the correlation coefficients for the same temporal resolution. Besides, this approach zooms in more on the precipitation extremes, as 2-day maxima are considered. Keep into mind that the maxima of the different variables do not have to be measured at the same moment in time. On the one hand, this can enhance correlations in a biased way, if the hours are widely spaced in time. On the other hand, lower correlation coefficients are found when we look at correlation between 2-day maxima in P and the variables T, TD, and FH, measured at the time of the maximum in P. Negative feedbacks are then assumed to play a role, as for example cooling of the air due to precipitation. We observe the same relationships which are relative high as in Figures \ref{fig:Heatmap_allp}, but than stronger, just like in \ref{fig:CAPE_p}. This shows that the relationships are stronger for the extremes. Correlation coefficients between 0.28 and 0.36 between the 2-day maxima in hourly precipitation sums (Pmax2d) and 2-day maxima in T and Td. Hence,the theory in which precipitation intensity of more intense precipitation events, relate to the ability of air to hold moisture, matches with the relatively high correlation coefficient (0.38) between 2-day maxima in PI and TD, with the latter as measure of moisture. Even higher coefficients (0.32-0.38) are found when the 2-day maxima in precipitation intensities are considered instead of Pmax2d. The correlation coefficients between precipitation 2-day maxima and CAPE are lower, 0.11-0.14. In fact, the largest changes in magnitude of correlation coefficients are detected for FH values. Whereas, no strong correlation are found between all-hour FH and other variables, except with RH, the 2-day maxima in FH correlate well with the precipitation and temperature maxima. Especially the correlation between the 2-day maxima in FH and the 2-day precipitation is high (0.44). 2-day precipitation sum correspond more to long-lasting precipitation events and maxima in hourly precipitation sum and intensity to short-lasting events. Besides, long-(short)-lasting events are generally characterized by events of a larger (more local) scale. Therefore, it could be explained by the synoptic settings corresponding to high 2-day precipitation sums. Stable high pressure area's are associated with stable weather conditions and maybe local convective showers, while unstable low pressure area's are associated with unstable conditions and passing frontal systems. A higher pressure difference results in higher wind speeds. In short, the correlation between 2-day precipitation sums and 2-day maxima in wind speeds could be explained by the synoptic setting. On the contrary, the relationship between FH and RH is weaker for the maxima. Futher, we investigate whether we get an even stronger signal if we look at maxima of wet days. This would give a two times higher resolution, but lower extremes are then considered. The results are shown in Figure \ref{fig:Heatmap_maxip}, were maxima in CAPE are excluded, and in Figure \ref{fig:CAPE_p} were daily maxima in CAPE are considered. We do not observe notable stronger correlations for the daily maxima, compared to the 2-day maxima, except for CAPE.
...
M_trialB <- subset(CAPEmax2d_mat, select=c("Pmax2d","Tmax2d", "TDmax2d","CAPEmax2d","FHmax2d")) #, "PImax2d"
plot(M_trialB)
@
  \caption{Points clouds showing the type of relationship between the wet 2-day maxima of the following variables: temperature(T), dewpoint temperature(TD), convective available potential energy (CAPE), wind strength (FH), and hourly mean precipitation intensity (P), measured in De Bilt. Note, the x- and y-axes differ in scale.}
  \label{fig:pointclouds}
\end{figure}

Figure \ref{fig:pointclouds} confirms the linear relationship between (maxima in) T and Td, which we already suggested.  Multiple point clouds indicate on semi-exponential and exponential relationships. With semi-exponential we mean that aswell a linear shape, as an exponential shape can be recognized. This is the case for 2-day maxima in P in relation tot 2-day maxima in T or Td. On contrast, the 2-day maxima in CAPE relate to all other variables clearly in an exponential way. The 2-day maxima in FH relate to P and CAPE maxima in a more exponential form, while the relations with 2-day maxima in T and Td do not have a distinctive form. Nontheless, it is apparent that linearity can not be assumed for most of the relationships. Besides, another assumption for pearson correlation coefficients is normal-distributed variables (REF method). In Chapter 3 (REF CH3) we already observed non-normal probability density functions for all hours and 2-day maxima in P. However, the Spearman correlation coefficient does not assume normal-distributed data, and is a measure of non-linear relationships (REF method). Figure \ref{fig:Spearmanheatmap} shows two heat maps, one for all wet 2-day maxima of all variables, except CAPE, and one for the variables in relation to CAPE for De Bilt only. As shifting to a daily maxima did not result in improvement for the previous Figure, we do not include dialy figures for this approach.
...
@
    ...
    \label{fig:CAPE_sp}
  \end{minipage}
  \caption{Heatmap consisting of Spearman correlation coefficients of the variables: intensity (I), wind strength (FH), measure of relative humidity (RH), hourly precipitation sum (P), temperature(T) and dewpoint temperature(TD).The heatmaps are computed from: 2-day maxima of all variables (meaned over all stations), except CAPE (a), 2-day CAPE maxima and daily CAPE maxima in relation to other variables for de Bilt (b). $N_{d2}$ is the 2-day count of CAPE measurements. Colouring from blue to red gives the sign and strength of the relationship as is shown in the legend.}
...
The wind speed is both for 2-day precipitation sums, and for 2-day maxima in P and PI the strongest explanatory factor (coefficients of 0.37-0.47). It's strange that although the relationships mainly seem to be non-linear that the Spearman correlation coefficients are not higher in general, than the Pearson correlations. Especially the 2-day maxima in CAPE correlate relatively weakly with precipitation maxima (0.07 and 0.1 with P and PI), However, the Spearman approach is more correct from a statistical point of view. As we except Spearman correlation coefficients  to be stronger for daily CAPE maxima, we plotted these also in Figure \ref{CAPE_sp}. The correlations are indeed higher for the dialy resolution compared to the 2-day resolution, even 2.5-3 times. Nevertheless, we want to explain changes in 2-day precipitation maxima, so we can only conclude that the influence of the 2-day CAPE maxima is likely to be not that high regarding the strength of the relationship.

To summarize, we find relatively high correlation coefficients for the factors T, Td, CAPE and FH of which we know from theory to have an influence on precipitation changes. These correlations are stronger for the extremes in these variables. Different forms of relationships (linear, exponential, combination of both) are recognized by point clouds. 2-day T and Td maxima relate in a semi-exponential form to precipitation maxima. CAPE and FH 2-day maxima have a distinctive exponential relationship with precipitation maxima. With respect to 2-day P, the Pearson correlations are higher for T and Td 2-day maxima (cor=0.33 and 0.36 respectively),than the Spearman correlations. This approach considers the strenght of linear relationships. The Spearman correlations describe non-linear relationships and are much higher and the highest of all explanatory variables for 2-day maxima in P corresponds to the 2-day maxima in FH (cor=0.4). Despite of the exponential dependancy between 2-day CAPE and precipitation maxima, corresponding Spearman correlations are lower than the Pearson correlations. Following 2-day maxima FH, the order of strength regarding the Spearman correlation with 2-day maxima in P (of which significant trends are found in Chapter 3 REF) is: TD (cor=0.3), T (cor=0.2), and lastly CAPE (cor=0.07).
...
In this section we investigate whether the key variables, wind strength, CAPE, temperature and dewpoint temperature, regress in time. First, we plot the probability density functions in order to choose the appropriate method(s) of trend analysis for each variable, i.e. linear regression on normal distributed data and quantile regression for both normal and non-normal distributed data. Second, we plot the fitted quantile and linear regression lines for all 2-day maxima. In order to detect the common trend for these variables, we have meaned these maxima over the 5 stations. (The resulting trends can are then compared based on their appearance and rates to the trends in 2-day maxima in hourly precipitation intensity. --> if we add relative trends in Chapter 3 and here)  %(and frequency of wet hours and 2-day precipitation sum)
We do not differentiate on summer and winter trend in the trend analysis. However, in Section \ref{subsub:changdist} we will consider seasonal dependancy, next to time dependancy of the different relationships. Note, trend analysis on 2-day CAPE maxima is applied for the time period for which CAPE data is available (1993-2015) and not meaned over stations, as only CAPE data from de Bilt is available. We do not study in depth the historical changes in instrumentation and measuring sites as we did for precipitation, due to the time restriction of this research. Third, we apply a significance test on the trends.
...
\begin{figure}[H]
  \centering
  \textbf{Probability density functions of 2-day maxima}\par\medskip
  \begin{minipage}{0.48\textwidth}
<<label=dist_FHmax2d>>=
...
<<label=dist_Tmax2d>>=
load("./inst/tussenStap/allmax2d_mean.rda")
plot(density(allmax2d_mean$Tmax2d),col="red",lwd=3, main=NA,   #Distribution of nr of wet hours
...
@
  ...
  \begin{minipage}{0.48\textwidth}
<<label=dist_TDmax2d>>=
plot(density(allmax2d_mean$TDmax2d),col="purple",lwd=3, main=NA,   #Distribution of nr of wet hours
...
load("./inst/tussenStap/CAPE2dmax.rda")
plot(density(CAPE2dmax$CAPEmax2d),col="darkgreen",lwd=3, main=NA,   #Distribution of nr of wet hours
      xlab="2-day max CAPE (J/kg)", ylab="Probability density")
@
    \subcaption{2-day max CAPE}
    \label{fig:dist_CAPEmax2d}
  \end{minipage}
\caption{Probability density functions of 2-day maxima in wind strength (a), temperature (b), dewpoint temperature (c) meaned over all 5 stations, and 2-day maxima in convective available potential energy for only De Bilt (d). The probability density function of FH is computed from 2-day FH maxima between 1993-2015, to deal with the difference in rouding due to automatization around the year 1993. The other probability density functions are computed from data between 1958-2015. Note, the x- and y-axes differ in scale per variable.}
\label{fig:pdfall}
\end{figure}

All the probability density functions shown in Figure \ref{fig:pdfall} are computed from the 2-day maxima between 1958-2015, except for the probability density function of the FH maxima. Before the automatization of wind speed measurements (in 1993), data was not rounded to 1 m/s. As two types of data (rounded and non-rounded) give a strange appearance of the probability density function, we decided to only plot the 2-day FH maxima measured from 1993 onwards. However, the form of this probability density function is not different from the probability density function consisting of all FH maxima. The 2-day maxima in FH, T and TD are distributed in a more normal way, compared to 2-day maxima in CAPE which are distributed more in a non-normal way (Figure \ref{fig:pdfall}). Note, that the probability density function of wind speed is not entirely symmetric. For this difference in probability density functions, we only apply linear regression on the 2-day maxima of FH, T and TD. For 2-day CAPE maxima we study the behaviour of the yearly mean of 2-day maxima, which is normal-distributed. For all data we study the quantile regression fits on the 2-day maxima. As precipitation extremes are stronger correlated to the extremes in the explanatory factors, the trends in the higher quantiles are the most interesting. The results of the trend analysis for the 4 variables are listed here.
...
@
  ...
  \end{minipage}
\caption{Quantile regression lines with $\tau = 75, 90, 95, 99, 99.9$ \%  of 2-day maxima in wind speed meaned over all stations (a), the corresponding legend and yearly rates in FH in m/s/yr (b). The black line provides the linear regression fit of the 2-day FH maxima.}
\label{fig:Trends_FH}
\end{figure}
...
First, Figure \ref{fig:Trends_FH]} presents quantile and linear regression fits for 2-day maxima in wind speeds and the corresponding rates. A negative signal is detected for the 2-day maxima in FH. For the period 1958-2015 the linear regression of 2-day maxima has the exact same negative slope as the 50\%-quantile regression line; -0.014 m/s/yr (Figure \ref{fig:yr_rates_FH}). % 0.014/8.75*100 = -0.16% /yr   = for 58 years -9.1%
...
(Maybe add: Decreasing of the wind speed for the Netherlands is consistent with the study of;
- Smits, Klein Tank and Konnen, 2005  (1962-2002)
but not here in results maybe? where?)

The reason for a negative signal in wind speed maxima is not investigated in this thesis, but might be determined by changes in the position of the jet stream. A negative signal in 2-day wind speed maxima is not expected to explain the observed trends in precipitation intensity, as we know from theory that high wind shear (of which horizontal surface wind is one component) favors extreme precipitation via storm development and faster propagation of frontal systems over a certain site (Section \ref{subsec:theory}).
...
@
  ...
  \end{minipage}
\caption{Quantile regression lines with $\tau = 75, 90, 95, 99, 99.9$ \%  of 2-day maxima in temperature meaned over all stations (a), the corresponding legend and yearly rates in T maxima in $^{\circ}$C/yr (b). The black line provides the linear regression fit of the 2-day T maxima.}
\label{fig:Trends_T}
\end{figure}

Second, we studied the trends in temperature for the same period, of which the results are shown in Figure \ref{fig:Trends_T}. In contrast to CAPE and FH, positive quantile and linear regression trends are found for the 2-day maxima in T (Figure \ref{fig:QR_2dT}). What is more striking, is that for the quantile lines we can not hold the rule; the higher the quantile, the stronger the trend. Although, Figure \ref{fig:yr_rates_T} demonstrates that the strongest slope is detected for the 99\%-quantile (increase in 2-day maxima of 0.06 $^{\circ}$C/yr), % 0.05563636/27.5*100*58 = 0.2%/yr = for 58 yrs  11.73%
the weakest slope is detected for the 75\%-quantile (0.025 $^{\circ}$C/yr). Besides, not only the 90\%- and 95\%-quantile lines have a stronger slope than the linear regression line, but also the 25\%- and 50\%-quantile lines. The yearly linear regression of the 2-day T maxima gives an increase of 0.034 $^{\circ}$C/yr. % 0.034 / 12.75 * 100 = 0.27%/yr = for 58 yrs 15.5%
...
@
  ...
  \end{minipage}
\caption{Quantile regression lines with $\tau = 75, 90, 95, 99, 99.9$ \%  of 2-day maxima in dewpoint temperature meaned over all stations (a), the corresponding legend and yearly rates in TD maxima in $^{\circ}$C/yr (b). The black line provides the linear regression fit of the 2-day TD maxima.}
\label{fig:Trends_TD}
\end{figure}

Third, we applied trend analysis on the 2-day maxima in TD (see Figure \ref{fig:Trends_TD}). From Figure \ref{fig:QR_2dTD} we can deduce that 2-day maxima increase in time for the period 1958-2015. The linear regression line slopes relatively steeply with a rate of 0.025 $^{\circ}$C/yr. Only two quantiles have stronger slopes, the 25\%-quantile and the 95\% quantile. The slope of the 25\%-quantile is remarkably the highest; 0.031 $^{\circ}$C/yr. % 0.03058824/9*100 = 0.34 %/yr *58 = 19.7%
However, the confidence interval is relatively wide for the lower quantiles. Nevertheless, the lower boundary of the confidence interval is always above 0.012, so these trends are probably significantly positive.
...
@
  ...
  \end{minipage}
\caption{Quantile regression lines with $\tau = 75, 90, 95, 99, 99.9$ \%  of 2-day maxima in convective available potential energy meaned over all stations (a), the corresponding legend and yearly rates in CAPE in J/kg/yr (b). The black line provides the linear regression fit of the yearly mean in 2-day CAPE maxima.}
...
Finally, we repeat the quantile regression fitting for 2-day CAPE values measured between 1993 and 2015 for station De Bilt. From Figure \ref{fig:CAPEqr_yearly} we can deduce that CAPE values in De Bilt decrease for this period. The linear regression of the yearly mean in 2-day maxima has a negative slope of approximately -10 J/kg/yr (Figure \ref{fig:yr_rates_CAPE}), %= 2.9%/yr (intercept +- 350), is for 23 years, 66% decrease
which is stronger negative than the lower (25\%- and 50\%-) quantiles, but weaker negative than the higher (75\%-,90\%-,95\%,99\%) quantiles. The strongest trend is found for the 5\% highest 2-day CAPE maxima, which a decrease of 42 J/kg/yr. Furthermore, we can observe that the higher the quantile, the larger the confidence interval. The reason for this is that a lower samples size  belongs to a higher quantile, which enhances the uncertainty in the regression fit. The highest quantile lines (90-99\%-quantiles) are summer trends, as the winter CAPE values do not exceed 500 J/kg, this is shown in the next section (\ref{subsubsec:changdist}). At the end of this section we will investigate the significance of all the different quantile regression lines for all variables. Nontheless, we want to emphasize that observed negative trends are not able to explain the observed positive trends in hourly mean intensity. From theory we know that high CAPE values express the potential of strong updrafts, which are required for convective systems with intensive summer precipitation. On the contrary, we only consider trends in CAPE for the period 1993-2015 here, which can not be fully compared to trends in precipitation maxima for the period 1958-2015.
...
In order to investigate this, and also for the trends in the 2-day maxima of the other variables, we conduct a 9999 Monte Carlo permutation test. For wind speed (FH) and CAPE we tested whether the observed slope is significantly lower than all randomly permuted slopes in order to detect a negative trend, while for temperature (T) and dewpoint temperature (Td) we tested it vice versa in order to detect a significant positive trend. Table \ref{tab:p_rates_all} gives an overview of the significance of the trends. A red (green) marking of a cell indicates a decreasing (increasing) trend significant at the 95\% confidence level (CL). All the linear and quantile regression trends are significant, except for the 99\%-quantile of 2-day maxima in FH (p=0.94), of which we already discusses the large uncertainty expressed by the wide confidence interval for this quantile. In fact, all the trends are significant on the 99\% confidencence level, except for the 99\%-quantile for FH and CAPE. It should be stressed that the 2-day maxima in CAPE are based only on data of De Bilt and for the period 1993-2015. Due to this different temporal lenght of the data set the resolution is 60\% lower than for the 2-day maximain T, TD and FH. Nontheless, the 99\%-quantile trend for this variable is significant on the 95\% CL and the other quantile trends on the 99\% CL.
...
In short, in this section we detected significant (99\% CL) negative trends in wind speed and CAPE and significant (99\% CL) positive trends in temperature and dewpoint temperature for the ordinary least squares regression fits and almost all quantiles fits. Combining the observed sign of the trends in the key variables with theory about their relationship with extreme precipitation, only changes in temperature and dewpoint temperature remain as potential causes behind the trends in intensification of precipitation.


\subsubsection{Changing distribution or relations}
\label{subsubsec:changdist}
% IMPROVE, two periods
%Here we bundle data, as we divide all p

In this section we zoom in on seasonal and temporal differences in relationships between maxima in the key factors (maxima in T, TD and CAPE) by comparing summer and winter season and a begin (1958-1986) and an end period (1987-2015). First, we plot the relationship between the maxima in hourly precipitation sums and the maxima of other variables as point clouds differentiating on summer and winter. Second, we present the summer and winter frequency distributions of the variables. Third, we plot binned summer and winter quantile regression to express changes in precipitation maxima as a function of changes in the other maxima. In case of temperature and dewpoint temperature we compare the quantile regression lines to a Clausius Clapeyron scaling. For each of the three figures per relationship we compare between the first and second time period, in order to answer the question whether the changes in precipitation are caused by the changes in statistics of the key variable in question or by a changes in the relationship with that key variable. As binning and differentiating on period and season lowers the resolution greatly, we chose to use the daily maxima instead of the 2-day maxima.
...
In figure \ref{fig:PvsT}A the precipitation maxima of wet days only increase with maxima in temperature in a non-linear way. Clear differences are visible between winter and summer. The winter precipitation are generally lower and the points slope more gently with respect to temperature maxima. Both the summer and the winter data resembles a triangle shape, with a monotonic increase till approximately 25$^{\circ}$C summer temperature and 10$^{\circ}$C winter temperature and a seasonal decrease after these maxima. The seasonal characterstics and overall shape are visible for both periods, but there are also differences between the point clouds. On the one hand, the first period has more outliers ($>$32 mm/hr for summer data, $>$12 mm/hr for winter data, ), on the other hand the second period has more (and higher) summer intensities at temperatures $>$ 30 $^{\circ}$C and winter intensities at temperatures $>$ 10 $^{\circ}$C. This can be more easily observed in the summer and winter distributions of temperature for each period. This is demonstrated in Figure \ref{fig:PvsT}B, in which each bin the number of wet hours per season per period is counted. Summer and winter distributions for both periods have similar Gaussian shapes, but are differently positioned with respect to temperature. Futhermore, more wet hours exist in winter, than in summer, explaining the magnitude difference between the winter and summer distribution. For the first period the maxima in number of winter data per bin of 2 degrees celsius is at 6-8 $^{\circ}$C, while the summer data is concentrated around 16-18 degrees celsius. For the second period the maxima correspond to 8-10 $^{\circ}$C in winter, and to 16-18 $^{\circ}$C in summer. Although, the summer peak matches regarding the temperature interval, the peaks after the maxima are relatively higher for the second period, compared to the first period. The triangle shapes observed in the summer and winter data is likely to be influenced by the shape in the distribution. Next to the the higher the temperature, the higher the precipitation intensity, does also hold: the higher the sample size for a certain temperature bin, the higher the chance on an extreme intensity. However, keep into mind that the peaks in summer and winter precipitation intensity are then for higher temperature in the point clouds, than expected from the distributions. In order to remove any influence of how the temperature is distributed on the precipitation intensities, we also plotted the mean summer and winter precipitation maxima per bin. In Figure \ref{fig:PvsT}C the solid lines connect these means. Moreover, the 95\% and 99\% quantile lines are plotted as dashed and dotted lines to visualize the temperature relation with \enquote{high} extremes in precipitation. Mind, the logaritmic scale of the y-axis. The mean and quantile lines are compared to Clausius Clapeyron (CC) scaling, in order to investigate whether this precipitation-temperature relationship can be deduced directly from the CC equation, or whether other dynamics are involved. From Figure \ref{fig:PvsT}C can be deduced that mean and quantile lines quite nicely follow the Clausius-Clapeyron equation (given by gray lines with the same style in linetypes). With very light gray a super CC-scaling (twice what is expected from the CC equation) is indicated. Only for some parts, such as the 95\% and 99\% quantile lines for temperatures higher than 15 degrees, a super CC-scaling is found. What is striking is that this also holds for the winter mean and quantile lines for temperatures lower than 2 degrees for the first period. It might be still influenced by the amount of data in a bin. Significant differences between summer and winter P-T relationships are not found. When comparing the two periods, little differences are present for the upper parts of the winter and summer mean and quantile line. The winter lines regress further (in a flat way) with respect to temperature for the second half, compared to the first half. This is explained by the shift in the temperature distribution to higher winter values. The upper parts of the summer mean is less deviating from the CC-scaling and the upper parts of the quantile lines are relatively more aligned with the super CC-scaling. Nevertheless, changes in the distribution seem to be more clearly visible for this time period comparison, than changes in the P-T relationships.
...
Moreover, we also investigated changes in the point clouds, distributions and binned mean and quantile lines between the two seasons and time periods. For increasing dewpoint temperature we observe the same monotonic increase of precipitation intensity as for temperature. Besides, more outliers are present in the first half of the entire time period, while a shift is visible from relatively high counts of low winter and summer intensities in the first half to relatively high counts of high winter and summer intensities in the second half. As we know from Section \ref{subsubsec:correlationmatrices} that temperature and dewpoint temperature have a strong linear relationship, these similar features for T and Td are not unexcepted. However, regarding the binned mean and quantile lines for dewpoint temperature, relatively more parts are scaled to the super CC-scaling, compared to temperature. Nontheless, regarding the comparison of the two time periods, no notable changes in the relationship between P and Td are detected, while the dewpoint temperature distribution clearly shifts towards higher dew point temperatures.
...
plot_grid(plot1, plot2, plot3, labels=c("A", "B","C"), hjust=0, ncol = 1, nrow = 3)
@
  \caption{The maxima in Precipitation are plotted against FH values (A), B shows the distribution of FH maxima, and C demonstrates mean and the 95-percent quantile change in precipitation for every FH. FH is binned with steps of 1 m/s.}
  \label{fig:PvsFH}
\end{figure}


Furthermore, we investigated whether the summer and winter point clouds, distribution sand mean and quantile lines of the maxima in wind speed change in time. Although, we already know from Section \ref{subsec:trends} that the general long-term trend in FH maxima is not likely to explain to explain the change in precipitation maxima, it might be explaining a seasonal change. Nevertheless, the correlation of the precipitation maxima was highest with maxima in FH. In Figure \ref{fig:PvsFH} we plotted the result of this period and season comparison. Clear differences between summer and winter can be seen from Figures \ref{fig:PvsFH}A,B; summer is recognized by rlatively high precipitation maxima and low wind speeds, while for winter it is vice versa. Additionally the summer distribution has a more assymmetric shape with a higher compared, compared to the winter distribution, which has a more symmetric (Gaussian) and wider shape. Considering the time related change in distributions, we observe small decreases in the frequencies of the bars of winter maxima, but no distinct changes in the frequencies of summer maxima. Figure \ref{fig:PvsFH}C shows the change in the P-FH relationship. Although, the CC- and super CC-scaling do not have a physical meaning here, they are plotted so we can better compare the mean and quantile lines with those of temperature and dewpoint temperature. Regarding the slope of the lines, we can observe that large parts are aligned with the exponential CC-scaling, and some parts with the super CC-scaling. This only applies on winter mean and quantile lines, not on the summer lines. Nontheless, in general the slope is lower for the 95\%-quantile, compared to the mean, and for the 99\%-quantile compared to the 95\%-quantile. When considering the transition from the first to the second period, we observe in \ref{fig:PvsFH}A,B a decrease in the number of outliers in summer precipitation maxima (which was already observed in the T and Td plots), a slight lowering of the summer and winter bars corresponding to the higher wind speed maxima. From \ref{fig:PvsFH}C we deduce that there are less summer precipitation maxima for high wind speed maxima (pink lines are almost halved in length), and winter mean and quantile lines have weaker slopes for the higher wind speed maxima. Hence, a negative trend in FH maxima might match a change in winter precipitation better than in summer precipitation. Nontheoless, a physical explanation for intensification of precipitation due to decreasing wind speeds is not known from theory.

Finally, we would like to discuss changes in CAPE maxima and its relationship with precipitation maxima. However, we only have CAPE data from 1993 till 2015. Therefore, time comparison is not possible. Moreover, we only have data for De Bilt, so the resolution is too low to do the same binned plotting for precipitation maxima over CAPE maxima. We have plot the summer and winter point clouds providing some information about the seasonal P-CAPE relationship and the distributions of summer and winter CAPE maxima. The values of CAPE maxima are very seasonal dependent, as can been deduced from figure \ref{fig:PvsCAPE}. Winter data are confined to low CAPE maxima of $<$ 500 J/kg and low precipitation intensity maxima $<$ 10 mm/hr, whereas summer data has CAPE maxima up to 5000 J/kg and a much wider distribution in CAPE (and precipitation intensity) maxima. However, for both season a highly assymmetric (lognormal) distribution can be detected.
...
To summarize, we found strong seasonal differences in relationships between precipitation and T, Td, FH en CAPE maxima and in the distributions for each variable. In general, in summer we have relatively high daily maxima in temperature (mainly between 12-24$^{\circ}$), dewpoint temperature (most values between 9-20$^{\circ}$), and CAPE values (mainly between 0-2500 J kg$^{-1}$) and relatively low daily maxima in wind speed (2-12 m/s), compared to winter daily maxima (respectively 0-14$^{\circ}$,-2-12$^{\circ}$, 0-500 J/kg, 2-18 m/s). Time related changes in distributions show a

Positive exponential relationships, comparable with CC- and super CC-scaling are found for binned summer and winter P-T and binned winter P-FH mean and quantile lines. In contrast to maxima in wind speeds, summer and winter maxima in mean hourly intensity are similarly sensitive for changes in temperature and dewpoint temperature, except for the highest (summer) intensities which are less sensitive increases in temperature and dewpoint temperature. When comparing the second half of the time period to the first half, the P-T and P-TD relationships are slightly more aligned to the CC-scaling, while the P-FH quantile lines are less aligned from the CC-scaling. Moreover, the P-T and P-TD are as strong or stronger for the more extreme P maxima (95\%- and 99\%-quantiles), compared to the mean P maxima. This is vice versa for the P-FH relationship. However, for all three variables the timewise shift in distribution from higher summer and winter T and TD maxima and lower winter FH maxima is more clearly visible than changes in their relationships with P. Furthermore, we do not have a possible explanation for the increasing precipitation intensity as a result of negative trends in 2-day FH maxima (Section \ref{subsec:trends}). We namely expect from theory that reduced wind shear or weakened movement of frontal systems associated to lower horizontal wind speeds would reduce extreme precipitation. Nontheless, the strong relationship between P and FH maxima seems to be more a winter relationship (Figure \ref{fig:PvsFH}) and we observed the highest changes in precipitation intensities for the highest quantiles, corresponding to summer data.
...
- contrast in trends between coastal and non-coastal stations in presence of only summer and  only winter trends respectively. If this not due to coincidence, it might be due to a different mechanism behind the positive summer and winter trends. Besides, we already observed that linear regression of the precipitation sums for De Kooy and Vlissinge are quite similar, but mind that this accounts for precipitation sums on yearly resolution.
...
or rel cold air above sea, warms significantly (larger moisture availibility), (vs in winter air above sea is already relatively warm compared to land)
