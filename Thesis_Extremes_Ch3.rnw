...
\begin{figure}[H]
  \centering
  \textbf{Heat map of Pearson correlation}\par\medskip
  \begin{minipage}{0.48\textwidth}
<<Heatmap_all>>=
# All hours
heatmap(M)
...
@
  ...
  \end{minipage}
  \caption{A heatmap with Pearson correlation coefficients of the variables: intensity (I),hourly precipitation sum (P), temperature(T), dewpoint temperature(TD) and relative humidity. Colouring from blue to red gives the strength of the relationship as is shown in the legend.}
\end{figure}

To have an easy and clear overview of what kind of relationships there can exist between multiple variables,
we plotted a heatmap of correlation coefficient calculated with the Pearson method.
...
# 1) Maxima
CAPE_day <- Compare[N>0][(Year > 1993) | (Month > 2)][, list(maxCAPE = max(sbCAPE, na.rm = TRUE), RHsum=sum(RH), maxI=max(I), N=mean(N)), by=Date]
CAPE_day <- subset(CAPE_day,select=c(maxCAPE, RHsum, maxI, N))
@
...
<<Wind>>=
# relation hourly P sum and I with wind strength
# Colored with wind direction regime
#    c("N","NE","E","SE","S","SW","W","NW")
# Wind <- hrKNMI[DD<361]
# Wind[, DDregime := cut(DD,breaks=c(-1,seq(22,359,by=45),360),labels=c("N","NE","E","SE","S","SW","W","NW","N2"))]
# Wind <- within(Wind,DDregime[DDregime=="N2"] <- "N")
#
#
# ggplot(Wind) + geom_point(aes(x=FH,y=RH,colour=DDregime)) + geom_quantile(I~FH, tau=0.99))
#
# ggplot(Wind) + geom_bar(aes(x=FH))
# # lognormal
#
#
#
# M_RH[,RH_bin := .bincode(M_RH$RH, breaks=c(seq()))]
# wind direction
# 0:360 graden
# divide into N - NE - E - SE - S - SW - W - NW (8 regimes)
#
#
#
# hrKNMI[, meanP=mean(RH) ,  by=T]
#
...
For these important factors (maxima in T, TD and CAPE) we plotted point their distributions, and with respect to hourly precipitation sums point clouds and mean and quantile lines. We differentiated between summer and winter to have a better understanding of seasonal differences in precipitation explained by the key variables.
...
# Subset data set (to make calculations and plots faster)
# Choose temperature interval for which there are enough P measurements
M_TDmax <- subset(M_max,select=c(Pmax,TDmax,Season))[(Season=="Summer")|(Season=="Winter")]
Q_TDbin <- M_TDmax[(TDmax>-0.1) &(TDmax<18.1)][,TD_bin := .bincode(TDmax, breaks=seq(0,18,2),include.lowest=TRUE)]

# Calculate mean Pmax, 95 and 99% quantiles for each bin, per season
Q_TDbin <- Q_TDbin[,list(Pmeanmax=mean(Pmax),freq=.N,Q95=quantile(Pmax,probs=0.95),Q99=quantile(Pmax,probs=0.99)),by=list(TD_bin,Season)]
# Show only summer and winter season, and for bins with N, number of measurements, higher than 200
...
CAPE_day <- Compare[N>0][(Year > 1993) | (Month > 2)][, list(CAPEmax = max(sbCAPE, na.rm = TRUE),
