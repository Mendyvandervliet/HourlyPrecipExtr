...
\begin{figure}[H]
  \centering
  \textbf{Heat maps of Pearson coefficients for all wet hours}\par\medskip
...
@
  \caption{Points clouds showing the the wet 2-day maxima of the following variables: hourly mean precipitation intensity (P), temperature(T), dewpoint temperature(TD), convective available potential energy (CAPE), and wind strength (FH), measured in De Bilt. Note, the x- and y-axes differ in scale.}
...
@
  ...
  \end{minipage}
\caption{Quantile regression lines with $\tau = 75, 90, 95, 99, 99.9$ \%  of 2-day maxima in temperature averaged over all stations (a), the corresponding legend and yearly rates in T maxima in $^{\circ}$C/yr (b). The black line provides the ordinary least squares fit of the 2-day T maxima.}
\label{fig:Trends_T}
\end{figure}

Second, we studied the trends in temperature for the same period, of which the results are shown in Figure \ref{fig:Trends_T}. In contrast to CAPE and FH, positive quantile and ordinary least squares (OLS) trends are found for the 2-day maxima in T (Figure \ref{fig:QR_2dT}). What is more striking, is that for the quantile lines we can not hold the rule; the higher the quantile, the stronger the trend. Although, Figure \ref{fig:yr_rates_T} demonstrates that the strongest slope is detected for the 99\%-quantile (increase in 2-day maxima of 0.06 $^{\circ}$C/yr), % 0.05563636/27.5*100*58 = 0.2%/yr = for 58 yrs  11.73%
the weakest slope is detected for the 75\%-quantile (0.025 $^{\circ}$C/yr). Besides, not only the 90\%- and 95\%-quantile lines have a stronger slope than the OLS line, but also the 25\%- and 50\%-quantile lines. The OLS of the yearly means T maxima gives an increase of 0.034 $^{\circ}$C/yr. % 0.034 / 12.75 * 100 = 0.27%/yr = for 58 yrs 15.5%
...
@
  ...
  \end{minipage}
\caption{Quantile regression lines with $\tau = 75, 90, 95, 99, 99.9$ \%  of 2-day maxima in dewpoint temperature averaged over all stations (a), the corresponding legend and yearly rates in TD maxima in $^{\circ}$C/yr (b). The black line provides the ordinary least squares fit of the 2-day TD maxima.}
\label{fig:Trends_TD}
\end{figure}

Third, we applied trend analysis on the 2-day maxima in TD (see Figure \ref{fig:Trends_TD}). From Figure \ref{fig:QR_2dTD} we can deduce that 2-day maxima increase in time for the period 1958-2015. The ordinary least squares line slopes relatively steeply with a rate of 0.025 $^{\circ}$C/yr. Only two quantiles have stronger slopes, the 25\%-quantile and the 95\% quantile. The slope of the 25\%-quantile is remarkably the highest; 0.031 $^{\circ}$C/yr. % 0.03058824/9*100 = 0.34 %/yr *58 = 19.7%
...
@
  ...
  \end{minipage}
\caption{Quantile regression lines with $\tau = 75, 90, 95, 99, 99.9$ \%  of 2-day maxima in convective available potential energy averaged over all stations (a), the corresponding legend and yearly rates in CAPE in J/kg/yr (b). The black line provides the ordinary least squares fit of the yearly mean in 2-day CAPE maxima.}
...
\subsection{Changing distribution or relations}
