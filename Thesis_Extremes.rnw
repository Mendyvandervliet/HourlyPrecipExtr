...
# Select stations: 235 De Kooy, 260 De Bilt, 280 Eelde, 310 Vlissingen, 380 Maastricht
...
hrKNMI <- loaddata("./inst/extData/KNMI_rainhr.txt") # leave the file unchanged and skip the first 21 lines
save(hrKNMI, file="./inst/tussenStap/hrKNMI.rda")
@

<<loading, cache=FALSE>>=
load("./inst/tussenStap/hrKNMI.rda")
#nrKNMI <- within(hrKNMI,              # really nrKNMI ?
#                  STN <- factor(STN)) # can be done within loaddata and by STN := factor(STN)
@

<<AggregateByYear>>=
yearlyMax <- hrKNMI[, .(RHmax = max(RH)), by = .(STN, Year)]
@

<<SpatialComparisonViaViolinPlots, fig.cap="Violin plots of annual maximum hourly rainfall">>=
ggplot(yearlyMax, aes(x = STN, y = RHmax)) + geom_violin() +
