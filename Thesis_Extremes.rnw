...
\begin{figure}[H]
  \centering
  \textbf{PDF}
  % out.height='320px',out.width='400px'
<<pdfplot,cache=FALSE,fig.keep='high'>>=
...
%' \begin{figure}[H]
%'   \flushleft
%'   \begin{minipage}[adjusting]{0.40\linewidth}
%'
\begin{figure}[H]
  \centering
  \textbf{CDE}\par\medskip
  % out.width='400px'
<<cde_plot,cache=FALSE>>=
...
theme_bw()
@
  \caption{The conditional density estimate for all 5 stations: De Kooy (235), de Bilt(260), Eelde(280), Vlissingen (310) and Maastricht (380).}
  \label{fig:CDE}
\end{figure}
