...
@
  ...
  \label{fig:impact}
  \end{minipage}
  \caption{Quantile regression fits with $\tau = 50, 75, 90, 95, 99, 99.9$ \% on counts of wet hours a year, for all hours (a) and .., for all stations.}
  \label{fig:QR_freq}
\end{figure}
\hfill
\begin{figure}[H]
...
@
  ...
  \label{fig:Qfreq_sum}
  \end{minipage}
  \caption{Quantile regression fits with $\tau = 50, 75, 90, 95, 99, 99.9$ \% on counts of wet hours a year, for only Winter (a) and only Summer (b) data, for all stations.}
  \label{fig:QR_freqWS}
\end{figure}
