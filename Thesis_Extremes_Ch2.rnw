...
\begin{table}[ht]
  ...
  \hline\hline
    ...
    De Bilt   & 260 & -2.3e$^{-14}$ & 4.2e$^{-3}$ & 8.5e$^{-3}$ & - \\
    Eelde     & 280 &  - & 2.8e$^{-3}$ & - & - \\
...
As all the trends for the 99\%- and 99.9\%-quantile fits are negative, their rates are excluded from Table \ref{tab:rates_2daymax}. Take into mind, that although no significant trends are found for the highest quantiles, trends still can exist. A larger data set might give significant trends for the highest quantiles, which are assumebly positive, as all the other detected trends, except for the 50\%-quantile of De Bilt, are positive. The highest rate corresponds to the 95\%-quantile fit for De Kooy, which is on the order of 0.01 mm/hr/yr. After this trend, the three highest rates are those of De Bilt(90\%-quantile), Vlissingen(95\%-quantile) and Maastricht(95\%-quantile) on the order of 0.001-0.01 mm/hr/yr. For the entire period of 58 years this translates to increases in the extreme intensities of 0.06-0.6 mm/hr. All the other significant trends have rates on the order of 0.001 mm/hr/yr, expect for the 50\%-quantile line of the Bilt, of which the rate is 11 orders lower. In general, the higher the quantile, the higher the magnitude of the detected trend. This holds when only one station is considered. Comparing the rates of the 50\%-quantile lines of De Bilt and Vlissingen gives an absolute order diferrence of 17. The reason for this differences in rates or why the trend for the 50\%-quantile of de Bilt is negative, remains unclear. To investigate this and the spatial differences in the detection of signifcant trends, we should dive deeper into the mechanisms behind these trends (REF CH3)
...
% false p_S
%   STN   Q50s   Q75s   Q90s   Q95s   Q99s  Q999s
% 1: 235 0.9387 0.9924 0.9902 0.9347 0.3811 0.1020
% 2: 260 0.8652 0.9892 0.8468 0.8608 0.8971 0.4434
% 3: 280 0.4864 0.9818 0.9551 0.5009 0.3935 0.8345
% 4: 310 0.9742 0.7021 0.9612 0.9733 0.5980 0.1807
% 5: 380 0.8578 0.7312 0.9809 0.8568 0.4987 0.8072
...
\begin{table}[ht]
  ...
  \hline
  Station & STN & 50\%& 75\%                  & 90\%                  & 95\%                  & Q99\%& Q99.9\% \\
  \hline
    De Kooy     & 235 & 0.94                  & \cellcolor{green}0.96 & \cellcolor{green}0.98 & 0.70 & - \\
    De Bilt     & 260 & 0.87                  & \cellcolor{green}0.98 & 0.80                  & 0.85 & - \\
    Eelde       & 280 & 0.49                  & \cellcolor{green}0.98 & 0.93                  & 0.39 & - \\
    Vlissingen  & 310 & \cellcolor{green}0.97 & 0.75                  & \cellcolor{green}0.97 & 0.59 & - \\
    Maastricht  & 380 & 0.86                  & 0.82                  & \cellcolor{green}0.95 & 0.48 & - \\
  \hline
  \multicolumn{2}{c}{} \vline & \multicolumn{5}{c}{Winter}\\
  \hline
    De Kooy     & 235 & 0.63                  & 0.77                  & \cellcolor{green}0.96 & \cellcolor{green}0.98 & \cellcolor{green}0.98  \\
    De Bilt     & 260 & \cellcolor{green}0.99 & 0.90                  & \cellcolor{green}0.98 & 0.73                  & 0.72  \\
    Eelde       & 280 & 0.17                  & 0.94                  & \cellcolor{green}0.99 & 0.86                  & 0.57  \\
    Vlissingen  & 310 & \cellcolor{green}0.95 & 0.85                  & 0.76                  & 0.39                  & 0.06  \\
    Maastricht  & 380 & 0.83                  & \cellcolor{green}0.99 & 0.90                  & \cellcolor{green}0.97 & 0.35  \\
    \hline
  \end{tabular}
  \label{tab:p_int_S&W}
\end{table}
...
# Shift to resolution of values per 2 days  (= total hours /48 = 2542080/48= 52960 data points)
freq_wet2dS <- freq_wet2dS[,list(f=unique(f),Psum=unique(Psum),Year=mean(Year)),by=list(d2,STN)]
save(freq_wet2dS, file="./inst/tussenStap/freq_wet2dS.rda")

freq_wet2dW <- data2dW[, ":="(f=sum(code)),by=.(d2,STN)]
# Shift to resolution of values per 2 days  (= total hours /48 = 2542080/48= 52960 data points)
freq_wet2dW <- freq_wet2dW[,list(f=unique(f),Psum=unique(Psum),Year=mean(Year)),by=list(d2,STN)]
save(freq_wet2dW, file="./inst/tussenStap/freq_wet2dW.rda")
@

<<QR_freq2d_S&W,eval=FALSE>>=
QR_Sfreq2d <- QR_freqall(freq_wet2dS)
QR_Wfreq2d <- QR_freqall(freq_Wwet2dS)
...
ymin <- 0
ymax <- 30
plot1 <- QR_freqplotting(data=QR_Sfreq2d[STN == 235], title="De Kooy",ymin,ymax,Year=FALSE)
plot2 <- QR_freqplotting(data=QR_Sfreq2d[STN == 260], title="De Bilt",ymin,ymax,Year=FALSE)
plot3 <- QR_freqplotting(data=QR_Sfreq2d[STN == 280], title="Eelde",ymin,ymax,Year=FALSE)
plot4 <- QR_freqplotting(data=QR_Sfreq2d[STN == 310], title="Vlissingen",ymin,ymax,Year=FALSE)
plot5 <- QR_freqplotting(data=QR_Sfreq2d[STN == 380], title="Maastricht",ymin,ymax,Year=FALSE)
grid.arrange(arrangeGrob(plot1, plot2, plot3, plot4, plot5, Legendgrob5, ncol=2),
             ncol= 1)
@
...
ymin <- 0
ymax <- 30
plot1 <- QR_freqplotting(data=QR_Wfreq2d[STN == 235], title="De Kooy",ymin,ymax,Year=FALSE)
plot2 <- QR_freqplotting(data=QR_Wfreq2d[STN == 260], title="De Bilt",ymin,ymax,Year=FALSE)
plot3 <- QR_freqplotting(data=QR_Wfreq2d[STN == 280], title="Eelde",ymin,ymax,Year=FALSE)
plot4 <- QR_freqplotting(data=QR_Wfreq2d[STN == 310], title="Vlissingen",ymin,ymax,Year=FALSE)
plot5 <- QR_freqplotting(data=QR_Wfreq2d[STN == 380], title="Maastricht",ymin,ymax,Year=FALSE)
grid.arrange(arrangeGrob(plot1, plot2, plot3, plot4, plot5, Legendgrob5, ncol=2),
             ncol= 1)
@
...
Significance test
%TEST
<<p_testPsum,cache=TRUE>>=
p_Psum2d <- p_test(data=Psum,tau=0.50,Psum=TRUE)
setnames(p,c("STN","Q50"))
p_Psum2d[, ":="(Q75 = p_test(data=Psum,tau=0.75,Psum=TRUE)$V1,
         Q90 = p_test(data=Psum,tau=0.90,Psum=TRUE)$V1)]
p_Psum2d[, ":="(Q95 = p_test(data=Psum,tau=0.95,Psum=TRUE)$V1,
         Q99 = p_test(data=Psum,tau=0.99,Psum=TRUE)$V1,
         Q999 = p_test(data=Psum,tau=0.999,Psum=TRUE)$V1)]
...
\begin{table}[ht]
  ...
  \hline\hline
    ...
    Station   & STN & 50\% & 75\% & 90\% & 95\% & Q99\% & 99.9\% \\
    \hline
    De Kooy   & 235 &                   &                   0.07 & \cellcolor{green} 0.97 & \cellcolor{green} 1.00 & 0.85 & 0.07 \\
    De Bilt   & 260 &  \cellcolor{red}  & \cellcolor{green} 0.99 & \cellcolor{green} 1.00 &                   0.94 & 0.92 & 0.90 \\
    Eelde     & 280 &                   & \cellcolor{green} 0.95 &                   0.93 &                   0.92 & 0.72 & 0.26 \\
    Vlissingen& 310 &                   &                   0.07 &                   0.28 & \cellcolor{green} 0.95 & 0.83 & 0.46 \\
    Maastricht& 380 &                   &                   0.16 & \cellcolor{green} 0.99 & \cellcolor{green} 0.95 & 0.45 & 0.67 \\
     \hline
  \end{tabular}
