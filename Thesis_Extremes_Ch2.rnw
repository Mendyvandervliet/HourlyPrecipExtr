...
<<density>>=
ggplot(max2d[(max2d>0.9) & (Year==1962)], aes(x = max2d)) + geom_histogram(binwidth=1) + coord_trans(x = "log10") + facet_wrap(~STN)
...
freq_10YM <-  hrKNMI[(RH > 10) & (RH <20)][, ':='(f = .N),by = YM]
freq_10YM <-  freq_10YM[, list(YM=unique(YM)),by = f]
setkey(freq_10YM,YM)
#freq_10 <- QR_freqall(freq_10,STN=FALSE)
save(freq_5, file="./inst/tussenStap/freq_5.rda")
save(freq_10, file="./inst/tussenStap/freq_10.rda")
save(freq_10m, file="./inst/tussenStap/freq_10.rda")
@
