...
<<Example_DT,cache=FALSE>>=
dt <- data.table(x = c(1:100),y = rnorm(100))
@

<<Example_QR,cache=TRUE>>=
...
<<length,results='markup'>>=
length(QR_freq$Year)
@


% Significance test
<<p_testfreq,cache=FALSE>>=
hallo <- 1+1
