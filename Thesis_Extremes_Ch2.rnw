...
This chapter focusses on linear trends in hourly precipitation extremes. Whether the hourly intensity (calculated from the hourly precipitation sum as the mean rate of one hour) of extremes regresses linearly in time or not, is investigated for the period 1958 to 2015. Next to intensity, trends in the frequency of hourly extremes are considered. Both type of trends are compared to the regression of the mean hourly value in time, thereby studying the proportional change. In chapter (REF CH1) we concluded that spatial differences are in general not that high in the Netherlands, however, for extremes significant differences exist. Therefore, it is interesting to consider the trends individual for every station. Moreover, when a signal is present at multiple stations, this enhance the robustness of the trend observed.
...
From this figure we can not clearly observe trends for the different quantiles, except for the 99.9\% fit, corresponding to values higher than 11-27 mm/hr, occuring 0.1\% of the time ($\sim$ 9 hours per year). In fact, these trends differ highly between the five stations. As these trends are based on $\sim$ 510 data points it might be that the trends are not significant. We will discuss in more detail the significance of the plotted quantile lines at the end of this paragraph. Next to the amount of data points, trends can be influenced by biases. The assumption of independence of errors is violated by time series with an autocorrelation nature. The magnitude of the trend is influenced by serial correlation, when present %\ref{Yue2002}.
Therefore, we now investigate whether there is any autocorrelation and partial autocorrelation present in the hourly data.
...
pacf(hrKNMI[STN==260]$P,lag.max=80,main=NA)
 ...
 \end{minipage}
\vspace{1cm}
\caption{Autocorrelation and partial autocorrelation function for all hour precipitation data of station De Bilt. For the other stations we did not include the same figures as for both there are no significant differences between the stations.}
\end{figure}

For lags of 1 to 48 hours the autocorrelation and partial autocorrelation values are significant ($>$0.05). Moreover, a winter depression will not last longer than 2 days (and summer rain events are mainly on the scale of several hours). Therefore, we will now apply quantile regression on the maximum value for periods of 2 days to remove significant serial correlation(\ref{fig:QR_max2d}). Whereas in Figure \ref{fig:QR_allhr} only for the 99.9\% quantile lines linear regression was visible with the eye, in this plot the 99\% quantile line also seems to regress linearly next to the 99.9\%-quantile line. This arise from the new time scale. As here only the maxima of 2 days are considered, the 99.9\% is not the same as in the previous figure. In Figure \ref{fig:QR_max2d} the 99\%-quantile and 99.9\% are computed from a minimization function to values higher than 9-10 mm/hr and 16-20 mm/hr respectively. We can already deduce from this figure that spatial differences exist in the magnitude and type (positive/negative) of trend and that they are higher for the 99.9\%-quantile, than for the 99\%-quantile. The 99.9\% quantile line for de Kooy and Eelde decreases linearly in time with rates of 0.1 and 0.05 mm/hr/yr respectively, while the same quantile line increases for De Bilt and Maastricht (at rates of 0.13 and 0.07 mm/hr/yr) and remains almost constant for Vlissingen ($\sim$ 0.02 mm/hr/yr). No visible trends are observed for the yearly mean intensity and the 75\%,90\%,95\%- quantile.
...
max2d <- data2d[, list(max2d=unique(max2d), Psum=unique(Psum), Year=unique(Year),YM=unique(YM),month=unique(month)),by= list(d2,STN)]
...
Only for De Bilt Figure \ref{fig:QR_max2d} suggests that the 99 and 99 \% quantiles for regress considerable in time. Whether the 99 and 99.9 \% trends for de Bilt are significant is investigated by a 9999 Monte Carlo permutation test of the linear regression coefficient. The corresponding distribution plot of the permuted slopes is shown in Figure \ref{fig:MC_max2d}.
...
Next, to verify whether the 99.9\% quantile lines are indeed not significant for the other stations we will also study the corresponding p-values from the same permutation test. In fact, the significance of all quantile lines is determined for all stations and displayed in Table \ref{tab:p_table_2daymax}. A cell in the table is colored red, when a significant negative trend is present ($p \leq 0.05$). A green cell indicates a significant positive trend ($p \geq 0.95$).
...
All 99.9\% quantile regression fits are indeed insignificant. The suprising opposing regression lines for this quantile can thus not be detected as true trends, but may be influenced by the limited amount of data points. What is striking about these p-values, is that the quantiles for de Bilt are almost all significant (except for the 95\%-quantile), while for the other stations there are large differences in the p-values for the different quantiles. For Vlissingen the part of the distribution between the 50\%- quantile or higher than the 95\% and 99\%-quantiles, trends of increasing are visible. This suggests that the values between the 75\% and the 95\%-quantile do not increase (significantly). For Maastricht and De Kooy the same holds for the values higher than the 90\% and 95\%-quantile, but lower than the 99\% quantile. On the contrary, Eelde only has a significant increasing trend in the intensity for values higher than 75\%. Does this observation of trend differences in the intensity spectrum contradict to the current theory (IPCC SOURCE )of a shifting intensity distribution towards higher values as a result of temperature increase? Considering the method of quantile regression this does not need to be the case. In quantile regression there are no quantiles calculated for each time period, which would correspond to the distribution at that time. Quantile regression is namely a minimization function on the entire data set. Therefore, the fitted lines are not a result of a clear step-wise shift in time of the entire distribution and corresponding quantiles. (Each fitted quantile line can only be considered seperately.) Moreover, dissimilarity in trends for different quantiles is also observed in a study of Malik et al (citet REF Malik2016). This study also detects spatial inhomogeneity in trends for India. Despite the fact that the Netherlands is a relatively small compared to India, spatial differences are present in the magnitude and amount of significant trends. In Chapter 3 we hope to find an explanation for spatial differences in intensity. Table .. provides an overview of the rates of all significant trends. Although the trends are characterized by spatial inhomogeneity and dissimilarity with respect to the quantiles, it should be stressed that all significant trends detected are positive. In general, we can conclude that through a time period of 58 years the precipitation intensity of extreme events increases.
...
@
    ...
    \label{fig:Qint_sum}
  \end{minipage}
  \caption{Quantile regression fits with $\tau = 75, 90, 95, 99, 99.9$ \% on 2 day Winter (left) and Summer (right) maxima in hourly precipitation intensities (mm/hr), for all stations. The black line represents the OLS regression of the yearly means.}
  \label{fig:QR_int_WvsS}
\end{figure}

Figure \ref{fig:QR_int_WvsS} shows the quantile trends in 2 day maxima of both seasons. The 99\% and 99.9\%-quantiles are left out of this figure, as this would correspond to respectively 28.7 and 2.87 data points (only 1\% and 0.1\% of the 2 day maxima within 3 winter or summer months of 58 years).


What is striking, is the difference in magnitude of the extremes between summer and winter. Where the highest quantile represents values higher than 7.5-10 mm/hr for the winter data, the highest summer quantile corresponds to extreme intensities of 20-50 mm/hr. Not only the magnitude, but also the interspatial difference is much bigger for the summer season. Futhermore, the summer trends are, especially for the 99.9\% quantile much larger positive ( (0.517 mm/yr for Maastricht) or negative (-0.48 mm/yr for de Kooy), than the winter trends.

EXCLUDE 99.9\% !!  only 2.75 datapoints
Nevertheless, it is questionable how significant the 99.9\% quantile trends are for both seasons, in view of the further reduction in number of sample by taking  The 99\% quantile in this figure correspond to

Similar to Figure \ref{fig:QR_max2d}  . trends. .? for which station ? summer or winter dominated in overall trends? The significance of the quantile lines is tested and the corresponding p-values are showed in Table \ref{tab:p_int_S&W}.
...
\begin{table}[ht]
  ...
  Station & STN & 50\% & 75\% & 90\% & 95\% & Q99\% & 99.9\% \\
  \hline
    De Kooy     & 235 & 0.93 & \cellcolor{green}0.97 & \cellcolor{green}0.99 & \cellcolor{green}0.95 & 0.42 & 0.11 \\
    De Bilt     & 260 & 0.89 & \cellcolor{green}0.98 & 0.87                  & 0.79                  & 0.89 & 0.29 \\
    Eelde       & 280 & 0.92 & \cellcolor{green}0.95 & \cellcolor{green}0.95 & 0.66                  & 0.94 & 0.82 \\
    Vlissingen  & 310 & 0.91 & 0.78                  & \cellcolor{green}0.95 & \cellcolor{green}0.97 & 0.80 & 0.16 \\
    Maastricht  & 380 & 0.21 & 0.71                  & 0.90                  & 0.78                  & 0.54 & 0.80 \\
  \hline
  \multicolumn{2}{c}{} \vline & \multicolumn{6}{c}{Winter}\\
  \hline
    De Kooy     & 235 & 0.67                  & 0.18                  & \cellcolor{green}0.96 & \cellcolor{green}0.97 & 0.59 & 0.57 \\
    De Bilt     & 260 & \cellcolor{green}1.00 & 0.94                  & \cellcolor{green}0.97 & 0.65                  & 0.81 & 0.31 \\
    Eelde       & 280 & 0.09                  & \cellcolor{green}0.96 & \cellcolor{green}0.99 & 0.84                  & 0.52 & 0.45 \\
    Vlissingen  & 310 & \cellcolor{green}0.96 & 0.89                  & 0.79                  & 0.37                  & \cellcolor{red}0.05 & 0.25 \\
    Maastricht  & 380 & 0.84                  & \cellcolor{green}0.99 & 0.93                  & \cellcolor{green}0.97 & 0.30 & 0.47 \\
    \hline
  \end{tabular}
  \label{tab:p_int_S&W}
\end{table}
...
! elaborate
For boths seasons significant trends are demonstrated for De Kooy for the extreme intensities (75\%-99\%-quantiles).De Bilt in winter more significant trends.. for the 50\% and 90\%-quantiles,  in summer only the 75\%-quantile. The data for Eelde shows significant trends in both the summer, as the winter for values higher than the 75\% and 90\%-quantiles. Vlissingen only %\enquote{low} extremes in winter, while \enquote{high} extremes in summer.
No significant summer trends are detected for Maastricht, whereas the summer 75\% and 95\%-quantile lines are significant.
...
freq_wet2d <- freq_wet2d[,list(f=unique(f),Psum=unique(Psum)),by=list(d2,STN)]
...
# warning=false ; because we remove points which are not fitted in the plot by ymax
ymin <- 0
ymax <- 15
...
The distribution of the 2 day frequencies of wet hours (\ref{fig:dens_freq2d}) looks lognormal, therefore quantile regression is prefered above the linear regression. No evident trends are visible in this figure. Nontheless, all trends seems to be negative, with rates of an order of 1e${-4}$-1e${-5}$ number of wet hours per 2 days ($\sim$ ). 88\% of the hours is dry, so approximately 6 hours every 2 days are wet. As the number of wet hours per 2 day is most of the time small, we were interested whether stronger signals can be found, when studying the amount of wet hours a year. Figure \ref{fig:QR_freqyr} gives regression lines for yearly frequencies of wet hours.
