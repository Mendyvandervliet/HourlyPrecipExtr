...
save(data2d, file="./inst/tussenStap/max2d.rda")
@
...
data2d$YM <- format(max2d$Date,format="%Y-%m")
...
# MCperm[1] <- MC_coefperm(x=max2d$d2, y=max2d$max2d,tau=)
# save(MCperm, file="./inst/tussenStap/MCperm.rda")
@

% Load QR-fitted data
<<loading4, cache=FALSE>>=
# load("./inst/tussenStap/MCperm.rda")
@
