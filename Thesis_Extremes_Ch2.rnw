...
To investigate the frequency of extreme events, we will look at the frequency of wet hours with an intensity between 5-10, 10-15, 15-20 and $>$ 20 mm/hr.

 <<label=freqthreshold,cache=TRUE>>=
freq_5 <-    hrKNMI[RH > 5 & RH <10][ , ':='(f = .N),by = list(Year,STN)]
freq_5 <- QR_freqall(freq_5)
freq_10 <-  hrKNMI[RH > 10 & RH <15][ , ':='(f = .N),by = list(Year,STN)]
freq_10 <- QR_freqall(freq_10)
freq_15 <-  hrKNMI[RH > 15 & RH <20][ , ':='(f = .N),by = list(Year,STN)]
freq_15 <- QR_freqall(freq_15)
freq_20 <-  hrKNMI[RH > 20][ , ':='(f = .N),by = list(Year,STN)]
freq_20 <- QR_freqall(freq_20)
...
grid.arrange(arrangeGrob(plot1, plot2, plot3, plot4, plot5, Legendgrob2, ncol=2),
             ncol= 1)
@
    \subcaption{Hours with intensity 10-15 mm/hr}
...
plot1 <- QR_freqplotting(data=freq_15[STN == 235], title="De Kooy")
plot2 <- QR_freqplotting(data=freq_15[STN == 260], title="De Bilt")
plot3 <- QR_freqplotting(data=freq_15[STN == 280], title="Eelde")
plot4 <- QR_freqplotting(data=freq_15[STN == 310], title="Vlissingen")
plot5 <- QR_freqplotting(data=freq_15[STN == 380], title="Maastricht")
grid.arrange(arrangeGrob(plot1, plot2, plot3, plot4, plot5, Legendgrob2, ncol=2),
             ncol= 1)
@
    \subcaption{Hours with intensity 15-20 mm/hr}
...
plot1 <- QR_freqplotting(data=freq_20[STN == 235], title="De Kooy")
plot2 <- QR_freqplotting(data=freq_20[STN == 260], title="De Bilt")
plot3 <- QR_freqplotting(data=freq_20[STN == 280], title="Eelde")
plot4 <- QR_freqplotting(data=freq_20[STN == 310], title="Vlissingen")
plot5 <- QR_freqplotting(data=freq_20[STN == 380], title="Maastricht")
grid.arrange(arrangeGrob(plot1, plot2, plot3, plot4, plot5, Legendgrob2, ncol=2),
             ncol= 1)
@
  ...
  \end{minipage}
  \caption{Quantile regression fits with $\tau = 25, 50, 75, 90$ \% on counts of wet hours a year, of intensity 5-10 (a), 10-15 (b), 15-20, and $> 20$ mm/hr, for all stations.}
  \label{fig:QR_freqthreshold}
\end{figure}
