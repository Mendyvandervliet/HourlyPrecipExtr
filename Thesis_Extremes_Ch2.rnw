...
<<SpatialComparisonViaViolinPlots, fig.cap="Violin plots of annual maximum hourly rainfall">>=
ggplot(yearlyMax, aes(x = as.factor(STN), y = RHmax)) + geom_violin() +
  xlab("Station number") + ylab("Annual max mm/hr")
@
Figure \ref{fig:SpatialComparisonViaViolinPlots} shows violin plots of annual maximum hourly rainfall at each station.

<<QuantileTrends, fig.cap="Trend in quantiles of annual maximum hourly rainfall">>=
ggplot(yearlyMax, aes(x = Year, y = RHmax)) + geom_point() +
  geom_quantile(formula = y ~ x) +
  facet_wrap(~STN) +
  xlab("") + ylab("Annual max mm/hr")
@
Figure \ref{fig:QuantileTrends} shows the trend in the 25, 50, and 75 percentile of annual maximum hourly rainfall.
