...
<<legend, cache=TRUE>>=
QR_hrKNMI[, list(yr_mean = mean(RH)), by = list(STN, Year)]
...
ymax <- 30
plot1 <- QR2d_plotting(data=QR_max2d[STN == 235], title="De Kooy",point=FALSE)
plot2 <- QR2d_plotting(data=QR_max2d[STN == 260], title="De Bilt",point=FALSE)
plot3 <- QR2d_plotting(data=QR_max2d[STN == 280], title="Eelde",point=FALSE)
plot4 <- QR2d_plotting(data=QR_max2d[STN == 310], title="Vlissingen",point=FALSE)
plot5 <- QR2d_plotting(data=QR_max2d[STN == 380], title="Maastricht",point=FALSE)
...
Legendgrob3 <- legendGrob(c("mean","25%","50%","75%", "90%"), pch=15,
                          gp=gpar(col=c("black","purple","blue","green","orange"),cex=1), ncol=2)
...
All fits, except for De Kooy, show a negative trend for the 50\% wettest years (in the amount of wet hours a year). A positive trend is visible for the very wet (>90\%) years of De Kooy, which is a summer trend as can be seen by comparing Figure \ref{fig:Qfreq_win} and \ref{fig:Qfreq_sum}. To investigate whether these trends are significant a 9999 Monte Carlo permutation is applied.
...
p_freq[, ":="(Q95 = p_test(data=QR_freq,tau=0.95,freq=TRUE)$V1, Q99 = p_test(data=QR_freq,tau=0.99,freq=TRUE)$V1, Q999 = p_test(data=QR_freq,tau=0.999,freq=TRUE)$V1)]
@
...
To investigate the frequency of extreme events, we will look at the frequency of wet hours with an intensity between 5-10, 10-15, 15-20 and >20 mm/hr. For more intense hourly values there is to little data.

 <<label=freqthreshold,cache=TRUE>>=
freq_5 <- hrKNMI[(RH > 5) & (RH <10)][, ':='(f = .N),by = Year]
freq_5 <-  freq15[, list(Year=unique(Year)),by = f]
setkey(freq_5,Year)
freq_5 <- QR_freqall(freq_5,STN=FALSE)

freq_10 <-  hrKNMI[(RH > 10) & (RH <15)][, ':='(f = .N),by = STN]
freq_10 <-  freq10[, list(Year=unique(Year)),by = f]
setkey(freq_10,Year)
...
#ymax <- 20
ggplot(freq_5, aes(x = Year, color=Quantiles)) +        # data
...
ggplot(freq_10, aes(x = Year, color=Quantiles)) +        # data
    geom_point(aes(y=f, color="Points"))+
    geom_line(aes(y=f25, color="25%")) +
    geom_line(aes(y=f50, color="50%")) +
    geom_line(aes(y=f75, color="75%")) +
