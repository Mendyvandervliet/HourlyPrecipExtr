...
Next, to verify whether the 99.9\% quantile lines are indeed not significant for the other stations we will also study the corresponding p-values from the same permutation test. In fact, the significance of all quantile lines is determined for all stations and displayed in Table \ref{tab:p_table_2daymax}.
...
Similar to Figure \ref{fig:QR_max2d}  . trends. .? for which station ? summer or winter dominated in overall trends?
...
It is quite common that zero wet hours are counted for a time unit of 2 days, while this never happens for the previous used time unit of a year. This explains that the 25\%-quantile is zero in this figure.
...
P > 75\%, 90\%, 95\%, 99\%
1. Make new data set for intensity (not as mean hourly rate), by combining DR and P --> calculate intensity
...
In this chapter we found increasing trends in the intensity for the heavier rain events at every station, while no trend was detected for the mean intensity. Moreover, we found a robust decreasing signal in the frequency of wet hours. In view of these trends it is interesting to know whether there are also changes in the total amount of precipitation through the same period, or that the two opposite trends cancel each other.
