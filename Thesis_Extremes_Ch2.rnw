...
Distribution of all 2day max intensities
<<Distribution>>=
plot(density(max2d$max2d), type='l', col='blue', lwd=3, main=NA,
              xlab="I(mm/hr)", ylab="Density")
@
...
For de Bilt, both the 99\%-quantile, as well as the 99.9\%-quantile are not significant (P=0.8734). This might be due to the limitation in the number of values higher than the 99\%- and 99.9\%-quantile ($\sim$ 106 and 11 data points). If the 99.9\% is not significant for all stations, this might explain the opposing signals.
...
Next, to verify whether the (99\% and) 99.9\% quantile lines are indeed insignificant for the other stations, we will also study the p-values for this fit for the other stations. In fact, the significance of all quantile lines is determined for all stations and displayed in Table \ref{tab:p_table_2daymax}. A cell in this table is colored red, when a significant negative trend is present ($p \leq 0.05$). A green cell indicates a significant positive trend ($p \geq 0.95$).
...
@
  ...
  \end{minipage}
  \caption{Distribution plot of all 2-day maxima in intensities for the period 1958-1970 (a) and for the period 2003-2015 (b).}
...
\begin{table}[ht]
  ...
  \hline\hline
    ...
    %\multicolumn{1}{c}{STN} &
    \vline
    & \multicolumn{4}{c}{Rates (mm/hr/yr)} \\
    \hline
...
As all the trends for the 99\%- and 99.9\%-quantile fits are insignificant, their rates are excluded from Table \ref{tab:rates_2daymax}. Take into mind, that although no significant trends are found for the highest quantiles, trends can still exist. A larger data set might give significant trends for the highest quantiles, which are then assumably positive, as all the other detected trends are positive (except for the 50\%-quantile of Eelde).The computation of the rates of all significant rates, does not only gives us the magnitude, but also verifies whether a trend is indeed positive or negative. The assumption that the distribution of slopes from the 9999-MC permutation test is positioned at zero can be invalid for very small trends. Therefore, when an observed slope is smaller (larger) than the 5\% (95\%) confidence level, it can still be positive (negative). This is the case for the 50\%-quantile trend for de Bilt, which is positive, but smaller than the 5\% confidence level.

The highest rate corresponds to the 95\%-quantile fit for De Kooy, which is on the order of 0.01 mm/hr/yr. After this trend, the three highest rates are those of De Bilt (90\%-quantile), Vlissingen (95\%-quantile) and Maastricht (95\%-quantile) on the order of 0.001-0.01 mm/hr/yr. For the entire period of 58 years this translatesto an increase of the intensity of extreme hourly precipitation of 0.06-0.6 mm/hr. All the other significant trends have rates on the order of 0.001 mm/hr/yr, expect for the 50\%-quantile line of the Bilt (of which the rate is 11 orders lower) and Eelde (of which the rate is 13 orders lower). In general, the higher the quantile, the higher the magnitude of the detected trend. This holds when only one station is considered. Comparing the rates of the 50\%-quantile lines of De Bilt and Vlissingen gives an absolute order diferrence of 17. The reason for this differences in rates or why the trend for the 50\%-quantile of de Bilt is negative, remains unclear. To investigate this and the spatial differences in the detection of signifcant trends, we should dive deeper into the mechanisms behind these trends (REF CH3)
...
No significant trends for 99\% and 99.9\%, so excluded
! Change p-tables + Look at changed table!
...
Legendgrob5 <- legendGrob(c("50%","75%","90%","95%","99%"), pch=15,
...
@
  ...
  \begin{minipage}{0.48\textwidth}
    \flushright
<<label=QR2d_freqplotting,cache=TRUE,warning=FALSE>>=
ymin <- 0
ymax <- 30
...
% RATES
The distribution of the 2-day frequencies of wet hours (\ref{fig:dens_freq2d}) looks lognormal, therefore quantile regression is prefered above the linear regression. In Figure \ref{fig:Qfreq2d} the 25\%-quantile is excluded, because the fitted values are always zero. This means that the 25\% driest 2 days have no wet hours at all. No evident trends are visible in this figure. Nontheless, all quantile lines, except for the 99\%-quantile line, seem to be constant or decreasing in time. Corresponding rates are of an order of 1e$^{-3}$-1e$^{-2}$ number of wet hours per 2 days. As 88\% of the hours is dry, approximately 6 hours every 2 days are wet. The decrease computed as a fraction of the wet time is -0.002 \% per 2 days. To investigate whether the observed trends are significant, a 9999 Monte Carlo permutation is applied. Of every fitted quantile regression line the corresponding p-value is listed in Table \ref{tab:p_2dfreq}. Further, we were interested whether stronger signals can be found, when we study the amount of wet hours a year. Figure \ref{fig:QR_freqyr} gives regression lines for yearly frequencies of wet hours.


% Significance test
<<p_testfreq2d,cache=TRUE>>=
...
\begin{table}[ht]
  \centering
  \caption{Rates of significant 2-day frequency trends. * significantly constant}
  \begin{tabular}{l|r|r|r|r|r|r|r}
  \hline\hline
    \multicolumn{2}{c}{Location}
    %\multicolumn{1}{c}{STN} &
    ...
    \hline
    De Kooy   & 235 &  -            & -3.1e$^{-17}$ & -3.1e$^{-17}$  & 6.1e$^{-17}$ & - & -\\
    De Bilt   & 260 & -0.02         & -0.02         & -2.5e$^{-16}$  & -            & - & -\\
    Eelde     & 280 & 0*            & -9.2e$^{-17}$ & -0.02          & 0*           & - & -\\
    Vlissingen& 310 & -3.8e$^{-17}$ & -3.8e$^{-17}$ & -              & -            & - & 0* \\
    Maastricht& 380 & -0.03         & -6.1e$^{-17}$ & -0.04          & -0.04        & - & - \\
     \hline
  \end{tabular}
  \label{tab:rates_freq2d}
\end{table}

%CHECK positive , negative color and sign rate!
%ADD interpretation

Between summer and winter there are some differences for the 2-day frequency data. For De Bilt and Eelde the decrease in 2-day wetness is recognized for summer data, while the fitted quantile lines for winter data are constant in time. The significant decreasing trends for all 2-day frequencies are found for both summer, and winter. The higher quantiles decrease stronger in summer, than in winter. However, the 75\%-quantile decreases stronger for winter, than for summer. Except for the winter 99\%-quantile (which is evidently decreasing), the changes for de Kooy are minimal. The summer quantiles rise a little bit,, but on a neglible order, while the winter quantiles remain zero. For Vlissingen no significant relatively large changes were detected for all 2-day frequencies, which is also the case when both seasons are considered seperately.

%ONLY TEXT Summer and winter 2d


<<freq2d_S&W,eval=FALSE>>=
...
Moreover, we investigated trends of wetness by counting the number of wet hours a year, a winter and a summer. First, we started by studying the distribution of wet hours a year to determine which kind of regression approach is suitable.
...
\begin{figure}[H]
  \begin{minipage}{0.48\textwidth}
    \flushleft
<<label=QRW_freqplotting,cache=TRUE>>=
ymin <- 100
ymax <- 550
...
@
  ...
  \begin{minipage}{0.48\textwidth}
    \flushleft
<<label=QRS_freqplotting,cache=TRUE>>=
ymin <- 50
ymax <- 400
...
<<Distribution_Psum>>=
load("./inst/tussenStap/max2d.rda")
plot(density(max2d$Psum), type='l', col='blue', lwd=3, main=NA,
              xlab="Psum(mm/2day)", ylab="Density")
@

% For S: use max2dS,  for W: use max2dQ
<<Psum,cache=TRUE>>=
...
%     Eelde     & 280 &  0.46 & 0.64 &  \cellcolor{green}1.00 &                   0.98 & 0.93                  & 0.77 \\
...
# [1] 0.03908278
@
In order to investigate whether the increases in 2-day precipitation sums are mores summer or winter trends or both, we did the same analysis on summer and winter 2-day precipitation sums. In Figure \ref{fig:PsumS} we plotted the significant summer trends and in Figure \ref{fig:PsumW} the significant winter trends.
...
grid.arrange(arrangeGrob(plot1, plot2, plot3, ncol=2),ncol= 1)
@
    \subcaption{Summer}
    \label{fig:PsumS}
  \end{minipage}
...
plot2 <- ggplot(PsumW[STN == 280], aes(x = d2)) +        # data
  geom_line(aes(y=P95), color="blue") +
  ggtitle("Eelde") + # title
  ylim(c(ymin,ymax)) +
  scale_x_continuous(breaks=c(365.2,4017.7,7670.1,10409.4),
                     labels=c("1960","1980","2000","2015")) +
