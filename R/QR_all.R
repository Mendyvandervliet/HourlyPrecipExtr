...
QR_all <- function(data,var){
   ...
   if(var=="max2d"){
     tmp[, f50 := as.numeric(fitted.values(rq(max2d ~ d2, tau =0.5))),by=STN]
     tmp[, f75 := as.numeric(fitted.values(rq(max2d ~ d2, tau =0.75))),by=STN]
     tmp[, f90 := as.numeric(fitted.values(rq(max2d ~ d2, tau =0.90))),by=STN]
     tmp[, f95 := as.numeric(fitted.values(rq(max2d ~ d2, tau =0.95))),by=STN]
     tmp[, f99 := as.numeric(fitted.values(rq(max2d ~ d2, tau =0.99))),by=STN]
     tmp[, f999 := as.numeric(fitted.values(rq(max2d ~ d2, tau =0.999))),by=STN]
   }
