...
QR_freqall <- function(data){
  ...
  tmp[, f95 := as.numeric(fitted.values(rq(f ~ Year, tau =0.95))),by=STN]
  tmp[, f99 := as.numeric(fitted.values(rq(f ~ Year, tau =0.99))),by=STN]
  tmp[, f999 := as.numeric(fitted.values(rq(f ~ Year, tau =0.999))),by=STN]
