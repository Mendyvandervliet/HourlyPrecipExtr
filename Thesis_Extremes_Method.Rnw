...
A major disadvantage of permutation tests is that they can be very demanding in computational power, therefore it is worthwile to use a Monte carlo approach. With this approach we mean a repeated random sampling (with an specific order). So instead of calculating the slope of every possible order of a serie of almost 9570 observated values (would give 9570! outcomes). As the data set consist of so many points, the chances to have an identical ordered serie in 9999 random sampling is extremely low ($(9999-1)!\frac{1}{9570!}*\frac{1}{9570!} --> \frac{1}{9141!}).
