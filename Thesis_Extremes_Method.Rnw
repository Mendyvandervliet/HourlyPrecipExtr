...
In this thesis the variables investigated are listed in the following table.


table

name, unit - type of instrument - resolution

on hourly resolution includes precipitation in mm,
Precipitation intensity is defined as the mean hourly intensity in mm hr^{-1}, so the accumulated rain amount divided by 1 hour time. CAPE is computed from ... .
