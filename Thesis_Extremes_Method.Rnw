...
%
Mechanism
Starting with a theoretical framework in chapter 4 we decide which variables in our metadata can explain changes in precipitation. Moreover, we define key variables based on their correlation matrices and cross-correlation, on which quantile regression is applied (precipitation over key variable x) to focus on the extreme part of the precipitation distribution.
\end{document}
